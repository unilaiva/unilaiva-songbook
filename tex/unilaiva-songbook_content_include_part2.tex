...
%%
  ...
  % License: Public Domain (CC0)
  % Edited by: larva
  \mainchapter{Other}*[Hopi_Navajo_Zuni_circle_ed_by_larva__transbg_CC0_1200px.png]
    \begin{songs}{titleidx,authidx,tagidx}
  ...
  \mainchapter{Suomi}*[lintu_muinaissuomalaisilta_asuma-aloilta_perma__1426x1158px.png]
    \begin{finnish} % set language to finnish for hyphenation etc (polyglossia package)
      ...
      \section{Runoja, loitsuja}
        \begin{songs}{titleidx,authidx,tagidx}
          \ifchorded
            \newcommand{\waschorded}{true} % remember chorded state
            \newlength{\oldbaselineadj}
            \oldbaselineadj=\baselineadj % remember old value
            % songs: increase line spacing for better readability, not needed
            % in lyrics-only books, as they already set the baselines further
            % apart (this is done in internal-lyricbook-postsetup.tex)
          ...
          \ifdefined\waschorded % songs: back to the previous state
            \chordson
            \baselineadj=\oldbaselineadj
          \fi
        \end{songs}
    \end{finnish}
