...
from .constants import (
    ...
    RESULT_PRINTOUT_SUBDIRNAME, RESULT_IMAGE_SUBDIRNAME, RESULT_MIDI_SUBDIRNAME, RESULT_AUDIO_SUBDIRNAME,
    DEPLOY_IMAGE_SUBDIRNAME, DEPLOY_PRINTOUT_SUBDIRNAME, DEPLOY_AUDIO_SUBDIRNAME, DEPLOY_MIDI_SUBDIRNAME,
    DEPLOY_COMMONICON_SUBDIRNAME, DEPLOY_COMMONMETADATA_SUBDIRNAME, DEPLOY_COMMONOTHER_SUBDIRNAME,
    NODEPLOY_FNAMEPART,
)
...
def deploy_results(ui: UI, cfg: Config) -> None:
    ...
    for line in resultlist.lines():
        ...
        if cfg.deploy_common and ftype not in (
            RESULT_TYPE_COMMONICON, RESULT_TYPE_COMMONMETADATA,
    ...
    if skipped:
        ui.nodeploy_line(
            f"Skipped {skipped} existing identical file"
            f"{'s' if skipped != 1 else ''} during deploy"
        )
