...
\begin{document}
  ...
  \begin{titlepage}
    ...
    \vspace*{\fill}
    \begin{center}
      \vspace{-0.8em} % align whith "Contents" heading on the next page
  ...
  % The following image can be commented out (or made smaller), when the last
  % page of the TOC reaches an odd (recto) page. The idea is to show an image
  % rather than an empty page, because the next chapter starts on an even
  % (verso) page. Note that this page is quite prominent, so choose the
  % possible image carefully.
  \clearpage % this is needed
  \begin{center}%
    \vspace*{\fill}%
    % Original downloaded from: https://www.maxpixel.net/Musical-Instruments-Drum-Music-Jazz-Cat-1287910
    % Edited by: larva
