...
\fancypagestyle{unilaiva}[fancy]{%
  ...
  \fancyhead[CO]{%
    \headertitlestyle%
    \ifdefempty{\subbooktitle}{}{% not empty
...
\setcovericonsperm[ulsbs-tag-icon-partial-edition_EN_512x512px.png]%
\setcovericonsvar[ulsbs-tag-icon-chords_EN_512x512px]
  ...
  \ifdefempty{\coverpdf}
    {% \coverpdf is empty
      {% must be contained within a block for the settings not to bleed
        ...
        \begin{center}
          {\Huge \mainbooktitle}
          \ifdefempty{\bookmotto}{}{% not empty
        ...
        \iflyriconly% skip given set ii icons in lyric-only books
          \setcovericonsvar[\tagiconlyricsonly]%
        \fi%
        \insertcovericonshbc
      }
    }%
