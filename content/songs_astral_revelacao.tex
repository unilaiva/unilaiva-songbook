% Santo Daime: Revelação (by Simja)
% =================================
%
\audio[title={Soundcloud, studio recording}]{https://soundcloud.com/johannesbatista/sets/revelacao}

% The following sets the song number for the first song in this file.
% The number will automatically be incremented by one for each song.
% Please do not change this! Changing would make different versions of
% the songbook to have different numbers for the same songs, and it
% would totally mess up the selection booklets causing them to have
% wrong songs in them. (For the same reason, add new songs only to the
% end of each songs_ file.)
\setcounter{songnum}{0}


\beginsong{Sete Montanhas}[id={sete-montanhas-l7ywah},by={Emilio Dias},off={Diego Arregui e Céu da Neve},key={C},gk={}]
  \audio[key={C}]{https://soundcloud.com/johannesbatista/00setemontanhas}
  \beginverse\memorize
    \beginrep
      Lai |\[C]vem \[<*>]{} Sete Mon|tanhas
      Com \[F]seu cajado |\[C]forte
      Tra\[F]zendo o vento |\[C]norte
      Le\[F]vando todo |\[G]mal
      E pen\[<*>]samento de |\[C]morte
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu |^ve^-nho de Aru|anda
      Tra^zendo medi|^cina
      ^Paz e harmo|^nia
      A^mor e uni|^ão
      Para ^toda esta fa|^mília
    \endrep
  \endverse
  \vspace{10ex} % to force Lilypond onto the next page
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative g'' {
      \key c \major \time 4/4 \partial 4
      \repeat volta 2 {
        g4 \sectionmark "1." | c2~c8 g8 g g | e e4 e8 f f f f | e e4 \altcol e8 f8 f f f
        | e e4 e8 f f f f | g4 g8 g e e d d | c2 c4
        \break
        g'4 \posectionmark "2." | c2~c8 g8 g g | e e4 e8 f f f f | e e4. f8 f f f
        | e e4 e8 f f f f | g4 g8 g e e d d | c2 c4
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      Lai | vem Se -- te Mon -- | ta -- nhas;
      Com seu ca -- ja -- do | for -- te;
      Tra -- zen -- do~o ven -- to | nor -- te;
      Le -- van -- do to -- do | mal;
      E pen -- sa -- men -- to de | mor -- te.
      \set stanza = "2."
      Eu | ve -- nho de~A -- ru -- | an -- da;
      Tra -- zen -- do me -- di -- | ci -- na;
      Paz e har -- mo -- | ni -- a;
      A -- mor e u -- ni -- | ão;
      Pa -- ra to -- da~es -- ta fa -- | mí -- lia.
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4 | c1 | c2 f | c f
        | c f | g1 | c2.
      }
    }
    theChords = { \verseChords \verseChords }
    %\setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\setcounter{songnum}{1}
\beginsong{Na terra como no Céu}[id={na-terra-como-no-ceu-ww3u0e},by={Simja Johannes Lempinen},off={Jussarah},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/01-2n}
  \beginverse\memorize
    \beginrep
      Eu |\[Bm]brilho\[<*>], sou uma e|\[A]strela \[<*>]
      Assim na |Terra \[A7]{} como no |\[D]Céu \[<*>]
    \endrep
  \endverse
  \beginverse
    \beginrep
      A es|^trela que ^brilha lá em |^cima ^
      É a |Luz ^do meu cora|^ção ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Raio do |^mundo eu ^quero ir ao |^Eu ^
      Raiou no |mundo ^meu São Iri|^neu ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      O meu |^guia é o Pa^drinho Sebasti|^ão ^
      Eterna |vida encon^trei no cora|^ção ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Con|^fia con^fia na ale|^gria ^
      Assim na |Terra ^ como no |^Céu ^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative d'' {
      \key d \major \time 4/4 \partial 4.
      \repeat volta 2 {
        r8 d4 \sectionmark "1." | fis4 fis4. fis8 e d | fis4 e4.
        a,8 a cis | e4 e4. e8 d cis | d2~d8
      } \break
      \repeat volta 2 {
        r8 a8 \sectionmark "2." d8 | fis4 fis8 fis8 fis8 fis8 e d | fis4 e2
        a,8 cis | e2 e8 e8 d cis | d2~d8
      } \break
      \repeat volta 2 {
        a8 \sectionmark "3." a8 d8 | fis4 fis8 fis8 fis8 fis8 e d | e2~e8
        a,8 a8 cis | e4 e4 e8 e8 d cis | d2~d8
      } \break
      \repeat volta 2 {
        r8 a8 \sectionmark "4." d8 | fis4 fis8 fis8 fis8 fis16 fis16 e8 d | fis4( e4.)
        a,8 a8 cis | e8. e16 e8 e8 e8 e8 d cis | d2~d8
      } \break
      \repeat volta 2 {
        r8 d4 \posectionmark "5." | fis4 fis8 fis8 fis8 fis8 e8 d | fis4 e4.
        a,8 a cis | e4 e4. e8 d cis | d2~d8
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Eu | bri -- lho, sou~u -- ma es -- | tre -- la;
        As -- sim na | Ter -- ra co -- mo no | Céu.
      \set stanza = "2."
        A es -- tre -- la que bri -- lha lá em ci -- ma;
        É a luz do meu co -- ra -- ção.
      \set stanza = "3."
        Rai -- o do | mun -- do eu que -- ro ir ao | Eu;
        Rai -- ou no | mun -- do meu São I -- ri -- | neu.
      \set stanza = "4."
        O meu | gui -- a~é~o Pa -- dri -- nho Se -- bas -- ti -- | ão;
        E -- ter -- na | vi -- da en -- con -- trei no co -- ra -- | ção.
      \set stanza = "5."
        Con -- | fi -- a con -- fi -- a na~a -- le -- | gri -- a;
        As -- sim na | Ter -- ra co -- mo no | Céu.
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4. | b1:m | a | a2 a2:7 | d2~d8
      }
    }
    theChords = {
      \verseChords \verseChords \verseChords \verseChords \verseChords
    }
%     \setStaffSize #16 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Jardim de belas flores}[id={jardim-de-belas-flores-l0whto},by={Simja Johannes Lempinen},off={Matheus Serra},key={Em},gk={}]
  \audio[key={Em}]{https://soundcloud.com/johannesbatista/02-2j}
  \beginverse\memorize
    \beginrep
      É meu |\[Em]Deus e \[<*>]é meu |\[C]Mestre\[<*>]
      I|\[B7]ê ga\[<*>]lo can|\[Em]tou\[<*>]
    \endrep
  \endverse
  \beginverse
    \beginrep
      No Jar|^dim de ^belas |^flores^
      A minha |^Mãe foi ^quem man|^dou^
    \endrep
  \endverse
  \beginverse
    \beginrep
      O |^Mestre que ^me en|^sina^
      E a Ra|^inha que ^me ilu|^mina^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Cura |^cura ^cura |^tudo^
      Para |^eu po^der bri|^lhar^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Minha |^Mãe que ^vem co|^migo^
      Do |^alto ^do As|^tral^
    \endrep
  \endverse
  \beginverse
    \beginrep
      O Di|^vino ^Pai E|^terno^
      Para |^sempre a ^nós gui|^ar ^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative b' {
      \key e \minor \time 4/4 \partial 4
      \set melismaBusyProperties = #'() \slurDashed
      \repeat volta 2 {
        b8 \sectionmark "1." b | e4. dis8 e4 fis4 | g4 g2
        g4 | a4 g4 fis4 fis | e2.
      } \break
      \repeat volta 2 {
        b8 \sectionmark "2." b | e4. dis8 e4 fis4 | g4 g4.
        \altcol g8 g8 g8 | a4 g4 fis4 fis | e2.
      } \break
      \repeat volta 2 {
        b4 \sectionmark "3." | e4 e8 dis8 e4 fis4 | g4 g2
        g8 g8 | a4 g8 g8 fis4 fis | e4 e2
      } \break
      \repeat volta 2 {
        b8 \sectionmark "4." b | e4. dis8 e4 fis4 | g4 g2
        g8 g8 | a4 g4 fis4 fis | e2.
      } \break
      \repeat volta 2 {
        b8 \sectionmark "5." b | e4. dis8 e4 fis4 | g4 g2
        g4 | a4 g4 fis4 fis | e2.
      } \break
      \repeat volta 2 {
        b8 \posectionmark "6." b | e4. dis8 e4 fis4 | g4 g2
        g8 g8 | a4 g8 g8 fis4 fis | e2.
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        É meu | Deus e é meu | Mes -- tre;
        I -- | ê ga -- lo can -- | tou.
      }
      \set stanza = "2."
      \repeat volta 2 {
        No Jar -- | dim de be -- las | flo -- res;
        A mi -- nha | Mãe foi quem man -- | dou.
      }
      \set stanza = "3."
      \repeat volta 2 {
        O | Mes -- tre que me en -- | si -- na;
        E~a Ra -- | i -- nha que me~i -- lu -- | mi -- na.
      }
      \set stanza = "4."
      \repeat volta 2 {
        Cu -- ra | cu -- ra cu -- ra | tu -- do;
        Pa -- ra | eu po -- der bri -- | lhar.
      }
      \set stanza = "5."
      \repeat volta 2 {
        Mi -- nha | Mãe que vem co -- | mi -- go;
        Do | al -- to do As -- | tral.
      }
      \set stanza = "6."
      \repeat volta 2 {
        O Di -- | vi -- no Pai E -- | ter -- no;
        Pa -- ra | sem -- pre a nós gui -- | ar.
      }
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4 | e1:m | c | b:7 | e2.:m
      }
    }
    theChords = {
      \verseChords \verseChords \verseChords \verseChords \verseChords \verseChords
    }
    %\setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
%  %% Compact version, 2022-05-27
%   \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
%     theMelody = \relative b' {
%       \key e \minor \time 4/4 \partial 4
%       \set melismaBusyProperties = #'() \slurDashed
%       \repeat volta 2 {
%         b8( b) | e4( e8) d8 e4 fis4 | g4 g4. \altcol \parenthesize g8 \altmark "2." g8( g8)
%         | a4 g8( g8) fis4 fis | e4( e2)
%       }
%     }
%     theLyricsOne = \lyricmode {
%       \set stanza = "1."
%       \repeat volta 2 {
%         É meu | Deus __ _ e é meu | Mes -- tre
%         \skip 1 I -- _ | ê ga -- _ lo can -- | tou. __ _
%       }
%     }
%     theLyricsTwo = \lyricmode {
%       \set stanza = "2."
%       \repeat volta 2 {
%         No Jar -- | dim __ _ de be -- las | flo -- res
%         \altcol A mi -- nha | Mãe foi __ _ quem man -- | dou. __ _
%       }
%     }
%     theLyricsThree = \lyricmode {
%       \set stanza = "3."
%       \repeat volta 2 {
%         O __ _ | Mes -- tre que me en -- | si -- na
%         \skip 1 E~a Ra -- | i -- nha que me~i -- lu -- | mi -- na.
%       }
%     }
%     theLyricsFour = \lyricmode {
%       \set stanza = "4."
%       \repeat volta 2 {
%         Cu -- ra | cu -- _ ra cu -- ra | tu -- do
%         \skip 1 Pa -- ra | eu po -- _ der bri -- | lhar. __ _
%       }
%     }
%     theLyricsFive = \lyricmode {
%       \set stanza = "5."
%       \repeat volta 2 {
%         Mi -- nha | Mãe __ _ que vem co -- | mi -- go
%         \skip 1 Do __ _ | al -- to __ _ do As -- | tral. __ _
%       }
%     }
%     theLyricsSix = \lyricmode {
%       \set stanza = "6."
%       \repeat volta 2 {
%         O Di -- | vi -- _ no Pai E -- | ter -- no
%         \skip 1 Pa -- ra | sem -- pre a nós gui -- | ar. __ _
%       }
%     }
%     theChords = \chordmode {
%       \repeat volta 2 {
%         s4 | e1:m | c | b:7 | e2.:m
%       }
%     }
% %     \setStaffSize #14 % for better fit
%     \include "ulsbs-include-tail.ly"
%   \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Na presença do Senhor}[id={na-presenca-do-senhor-oxhlvn},by={Simja Johannes Lempinen},off={Kassem},key={Dm},gk={}]
  \audio[key={Bm}]{https://soundcloud.com/johannesbatista/03-2n}
  \transpose{3}
  \beginverse\memorize
    \beginrep
      Estou a|\[Bm]qui na pre\[<*>]sença do Se|\[F#7]nhor \[<*>]{} \altchords{\id[1]{alt. (Dm)}|F |A7 Gm6}
      Meu Di|\[A]vino \[<*>]Rei Cria|\[Bm]dor \[<*>]{} \altchords{|C |Dm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      É a |^Vós quem eu ^peço meu per|^dão ^ \altchords{\id[2]{alt. (Dm)}|B&maj7 |A7}
      Dai-me con|^forto e lim^pai meu cora|^ção ^ \altchords{|C |Dm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu vim a|^qui foi ^para me cu|^rar ^
      Estou a|^qui, meu ^Pai vai apu|^rar ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Dai-me |^força ^e Dai-me a|^mor ^
      Para se|^guir ^sempre com vi|^gor ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Para |^sempre, ^sempre estou a|^qui ^ \altchords{\id[4]{(Bm)}|Bm |F#7}
      Com minha |^Mãe^ juntinho a |^mim ^ \altchords{|A |Bm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Minha |^Mãe é quem ^dá estes en|^si^nos
      Para |^to^dos bem apren|^der ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Ôh Virgem |^Mãe sois tão ^bela e tão |^pu^ra
      O vosso |^brilho é que ^traz a santa |^cu^ra
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu peço a |^Vós ^no meu cora|^ção ^
      Aju|^dai a ^todos meus ir|^mã^os
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \transpose b d' \relative b' {
      \key b \minor \time 4/4 \partial 4.
      %\set melismaBusyProperties = #'() \slurDashed
      \repeat volta 2 {
        b8 \sectionmark "1." b d | fis4 fis8 fis fis fis e d | cis2.
        cis8 d | e4. e8 e4 d8 cis | b2~b8
      } \break
      \repeat volta 2 {
        r8 \sectionmark "2." b d | fis4 fis8 fis fis fis e d | cis2~cis8
        cis8 cis8 d | e4 e8 e e e d cis | b2~b8
      } \break
      \repeat volta 2 {
        b8 \sectionmark "3." b d | fis4. fis8 fis fis e d | cis2~cis8
        cis cis d | e4. e8 e e d cis | b2~b8
      } \break
      \repeat volta 2 {
        r8 \sectionmark "4." b d | fis4. fis8 fis fis e d | cis2~cis8
        cis cis d | e2 e8 e d cis | b2~b8
      } \break
      \repeat volta 2 {
        r8 \sectionmark "5." b d | fis4. fis8 fis fis e d | cis2~cis8
        cis cis d | e2~e8 e d cis | b2~b8
      } \break
      \repeat volta 2 {
        r8 \sectionmark "6." b d | fis4 fis8 fis8 fis fis e d | cis2 cis4
        cis8 d | e2 e8 e d cis | b2~b8
      } \break
      \repeat volta 2 {
        b8 \sectionmark "7." b d | fis4 fis8 fis8 fis fis e d | cis2
        cis8 cis cis8 d | e4 e8 e e e d cis | b2 b8
      } \break
      \repeat volta 2 {
        b8 \posectionmark "8." b d | fis2 fis8 fis e d | cis2.
        cis8 d | e4. e8 e e d cis | b2 b8
      }
    }
    % no repeats, as partial rest and repeats in lyrics don't work together well
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Es -- tou a -- | qui na pre -- sen -- ça do Se -- | nhor;
        Meu Di -- | vi -- no Rei Cri -- a -- | dor.
      \set stanza = "2."
        É a | Vós quem eu pe -- ço meu per -- | dão;
        Dai -- me con -- | for -- to~e lim -- pai meu co -- ra -- | ção.
      \set stanza = "3."
        Eu vim a -- | qui foi pa -- ra me cu -- | rar;
        Es -- tou a -- | qui, meu Pai vai a -- pu -- | rar.
      \set stanza = "4."
        Dai -- me | for -- ça e Dai -- me a -- | mor;
        Pa -- ra se -- | guir  sem -- pre com vi -- | gor.
      \set stanza = "5."
        Pa -- ra | sem -- pre, sem -- pre~es -- tou a -- | qui;
        Com mi -- nha | Mãe jun -- ti -- nho~a | mim.
      \set stanza = "6."
        Mi -- nha | Mãe é quem dá es -- tes en -- | si -- nos;
        Pa -- ra | to -- dos bem a -- pren -- | der.
      \set stanza = "7."
        Ôh Vir -- gem | Mãe sois tão be -- la e tão | pu -- ra;
        O vos -- so | bri -- lho~é que traz a san -- ta | cu -- ra.
      \set stanza = "8."
        Eu pe -- ço~a | Vós no meu co -- ra -- | ção;
        A -- ju -- | dai a to -- dos meus ir -- | mã -- os.
    }
    verseChords = \transpose b d' \chordmode {
      \repeat volta 2 {
        s4. | b1:m | fis:7
        | a | b2:m~8
      }
    }
    theChords = {
      \verseChords \verseChords \verseChords \verseChords
      \verseChords \verseChords \verseChords \verseChords
    }
    \setStaffSize #16 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
  %% Compact version, 2022-05-27
%   \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
%     theMelody = \transpose b d' \relative b' {
%       \key b \minor \time 4/4 \partial 4.
%       \set melismaBusyProperties = #'() \slurDashed
%       \repeat volta 2 {
%         \parenthesize b8 b d | fis4( fis8) \parenthesize fis fis fis e d | cis2( cis8)
%         \parenthesize cis cis d | e4( e8) \parenthesize e \parenthesize e( e) d cis | b2( b8)
%       }
%     }
%     theLyricsOne = \lyricmode {
%       \set stanza = "1."
%       \repeat volta 2 {
%         Es -- tou a -- | qui na pre -- sen -- ça do Se -- | nhor __ _
%         \skip 1 Meu Di -- | vi -- _ no Rei __ _ Cri -- a -- | dor. __ _
%       }
%     }
%     theLyricsTwo = \lyricmode {
%       \set stanza = "2."
%       \repeat volta 2 {
%         \skip 1 É a | Vós quem eu pe -- ço meu per -- | dão __ _
%         Dai -- me con -- | for -- to~e lim -- pai meu co -- ra -- | ção. __ _
%       }
%     }
%     theLyricsThree = \lyricmode {
%       \set stanza = "3."
%       \repeat volta 2 {
%         Eu vim a -- | qui __ _ foi pa -- ra me cu -- | rar __ _
%         Es -- tou a -- | qui, __ _ meu Pai vai a -- pu -- | rar. __ _
%       }
%     }
%     theLyricsFour = \lyricmode {
%       \set stanza = "4."
%       \repeat volta 2 {
%         \skip 1 Dai -- me | for -- _ ça e Dai -- me a -- | mor __ _
%         Pa -- ra se -- | guir __ _ \skip 1 sem -- pre com vi -- | gor. __ _
%       }
%     }
%     theLyricsFive = \lyricmode {
%       \set stanza = "5."
%       \repeat volta 2 {
%         \skip 1 Pa -- ra | sem -- _ pre, sem -- pre~es -- tou a -- | qui __ _
%         Com mi -- nha | Mãe __ _ \skip 1 \skip 1 jun -- ti -- nho~a | mim. __ _
%       }
%     }
%     theLyricsSix = \lyricmode {
%       \set stanza = "6."
%       \repeat volta 2 {
%         \skip 1 Mi -- nha | Mãe é quem dá es -- tes en -- | si -- nos
%         \skip 1 Pa -- ra | to -- _ _ dos bem a -- pren -- | der. __ _
%       }
%     }
%     theLyricsSeven = \lyricmode {
%       \set stanza = "7."
%       \repeat volta 2 {
%         Ôh Vir -- gem | Mãe sois tão be -- la e tão | pu -- ra
%         O vos -- so | bri -- lho~é que traz a san -- ta | cu -- ra.
%       }
%     }
%     theLyricsEight = \lyricmode {
%       \set stanza = "8."
%       \repeat volta 2 {
%         Eu pe -- ço~a | Vós __ _ _ no meu co -- ra -- | ção __ _
%         \skip 1 A -- ju -- | dai __ _ a to -- dos meus ir -- | mã -- os.
%       }
%     }
%     theChords = \transpose b d' \chordmode {
%       \repeat volta 2 {
%         s4. | b1:m | fis:7
%         | a | b2:m~8
%       }
%     }
% %     \setStaffSize #15 % for better fit
%     \include "ulsbs-include-tail.ly"
%   \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Meu Divino Pai Eterno}[id={meu-divino-pai-eterno-l0dymm},by={Simja Johannes Lempinen},off={Silvio},key={Dm},gk={}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/04-2m}
  \beginverse\memorize
    \beginrep
      Meu Di|\[Dm]vino \[<*>]Pai E|\[Gm]ter\[<*>]no \altchords{\id[1]{alt. (Dm)}|B&maj7 |E\textdegree}
      Quem me |\[A7]deu es\[<*>]te po|\[Dm]der \[<*>]{} \altchords{|A7 |Dm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Para |^mim com^preen|^der ^ \altchords{\id[2]{alt. (Dm)}|Dm |Gm6}
      Para eu |^poder ^rece|^ber \altchords{|F A7 |Dm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      A |^Gloria ^do meu |^Pai ^
      Para |^sempre res^plande|^cer ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Quem |^é meu ^Salva|^dor ^
      É seu |^Filho ^Reden|^tor ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu |^peço a ^meu Je|^sus ^
      A |^Luz que ^nos con|^duz ^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative d'' {
      \key d \minor \time 4/4 \partial 4
      \set melismaBusyProperties = #'() \slurDashed
      \repeat volta 2 {
        f8\sectionmark "1." e | d4. e8 f4 g | e2 e4
        e8 e | a4 g4 f4 e | d2.
      } \break
      \repeat volta 2 {
        f8\sectionmark "2." e | d4 \altcol e4 f4 g | e2.
        e8 e | a4 g4 f4 e | d2.
      } \break
      \repeat volta 2 {
        f4\sectionmark "3." \genmark "?" | d4. e8 f4 g | e2.
        e8 e | a4 g8 g8 f4 e | d2.
      } \break
      \repeat volta 2 {
        f4\sectionmark "4." | d4. e8 f4 g | e2.
        e8 e | a4 g4 f4 e | d2.
      } \break
      \repeat volta 2 {
        f4\sectionmark "5." | d4. e8 f4 g | e2.
        e4 | a4 g4 f4 e | d2.
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        Meu Di -- | vi -- no Pai E -- | ter -- no;
        Quem me | deu es -- te po -- | der.
      }
      \set stanza = "2."
      \repeat volta 2 {
        Pa -- ra | mim com -- pre -- en -- | der;
        Pa -- ra~eu | po -- der re -- ce -- | ber.
      }
      \set stanza = "3."
      \repeat volta 2 {
        A | Glo -- ria do meu | Pai;
        Pa -- ra | sem -- pre res -- plan -- de -- | cer.
      }
      \set stanza = "4."
      \repeat volta 2 {
        Quem | é meu Sal -- va -- | dor;
        É seu | Fi -- lho Re -- den -- | tor.
      }
      \set stanza = "5."
      \repeat volta 2 {
        Eu | pe -- ço~a meu Je -- | sus;
        A | Luz que nos con -- | duz.
      }
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4 | d1:m | g:m
        | a:7 | d2.:m
      }
    }
    theChords = { \verseChords \verseChords \verseChords \verseChords \verseChords }
    %\setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
%   %% Compact version, 2022-05-27
%   \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
%     theMelody = \relative d'' {
%       \key d \minor \time 4/4 \partial 4
%       \set melismaBusyProperties = #'() \slurDashed
%       \repeat volta 2 {
%         f8( e) | d4. \altcol \grace e8( \altmark "2." e8) f4 g | e2( e4)
%         e8( e) | a4 g8( g8) f4 e | d2.
%       }
%     }
%     theLyricsOne = \lyricmode {
%       \set stanza = "1."
%       \repeat volta 2 {
%         Meu Di -- | vi -- no Pai E -- | ter -- no
%         Quem me | deu es -- _ te po -- | der.
%       }
%     }
%     theLyricsTwo = \lyricmode {
%       \set stanza = "2."
%       \repeat volta 2 {
%         Pa -- ra | mim \altcol com -- pre -- en -- | der __ _
%         Pa -- ra~eu | po -- der __ _ re -- ce -- | ber.
%       }
%     }
%     theLyricsThree = \lyricmode {
%       \set stanza = "3."
%       \repeat volta 2 {
%         A __ _ | Glo -- ria do meu | Pai __ _
%         Pa -- ra | sem -- pre res -- plan -- de -- | cer.
%       }
%     }
%     theLyricsFour = \lyricmode {
%       \set stanza = "4."
%       \repeat volta 2 {
%         Quem __ _ | é meu Sal -- va -- | dor __ _
%         É seu | Fi -- lho __ _ Re -- den -- | tor.
%       }
%     }
%     theLyricsFive = \lyricmode {
%       \set stanza = "5."
%       \repeat volta 2 {
%         Eu __ _ | pe -- ço~a meu Je -- | sus __ _
%         A __ _ | Luz que __ _ nos con -- | duz.
%       }
%     }
%     theChords = \chordmode {
%       \repeat volta 2 {
%         s4 | d1:m | g:m
%         | a:7 | d2.:m
%       }
%     }
% %     \setStaffSize #15 % for better fit
%     \include "ulsbs-include-tail.ly"
%   \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Peço força do Céu}[id={peco-forca-do-ceu-2ast3v},by={Simja Johannes Lempinen},off={Risele},key={Bm},gk={},tags={valsa}]
  \audio[key={Am}]{https://soundcloud.com/johannesbatista/05-2p}
  \newchords{chords_pecoforca_a}\newchords{chords_pecoforca_b}
  \transpose{2}
  \meter{3}{4}
  \beginverse\memorize[chords_pecoforca_a]
    \beginrep
      Peço |\[Am]força do |\[F]Céu, da |\[A7]Terra e do |\[Dm]Mar \altchords{\id[1]{alt. (Bm)}|Bm |G |B7 |Em}
      Peço |força do |\[F]Vento pa|\[E7]ra me cu|\[Am]rar \altchords{| - |G |F#7 |Bm}
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_pecoforca_b]
    \beginrep
      Eu |\[Am]peço a meu |\[F]Pai, ve|\[A7]nha clare|\[Dm]ar \altchords{|Bm | - |B7 |Em}
      Pe|ço a minha |\[F]Mãe pra nos |\[E7]ilumi|\[Am]nar \altchords{| - |Cmaj7 |F#7 |Bm}
    \endrep
  \endverse
  \beginverse\replay[chords_pecoforca_a]
    \beginrep
      O |^Sol, a |^Lua, as Es|^trelas a gui|^ar \altchords{\id[2]{(Am)}|Am |F |A7 |Dm}
      Para |sempre esta |^Luz eterna|^mente a bri|^lhar \altchords{| - |F |E7 |Am}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_pecoforca_b]
    \beginrep
      E|^terna|^mente a |^nós gui|^ar \altchords{|Am |F |A7 |Dm}
      Tenho |a minha |^Mãe tambem |^tenho meu |^Pai \altchords{| - |F |E7 |Am}
    \endrep
  \endverse
  \beginverse\replay[chords_pecoforca_a]
    \beginrep
      Vou pe|^dindo e ro|^gando a to|^dos meus ir|^mãos \altchords{\id[5]{L (Bm)}|Bm |G |Em | \e}
      Va|mos com a|^mor rece|^ber nosso |^Pai \altchords{| - |D |F# |Bm}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_pecoforca_b]
    \beginrep
      Va|^mos se fir|^mar nes|^ta luz Di|^vina \altchords{|Bm |D | - |Em}
      Do nos|so Salva|^dor, de |^quem nos en|^sina \altchords{| - |Bm |F#7 |Bm}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \transpose a b \relative e'' {
      \key a \minor \time 3/4 \partial 4
      \repeat volta 2 {
        e8 \sectionmark "1." e8 | a4 a g | f2 e4 | d d c \genmark "?" | d2
        d8 d8 | d4 d e | c c c | b a gis | a2
      }
      \repeat volta 2 {
        e'4 | a, a b | c2 c4 | c c e | d2
        d4 | d d f | a2 e8 e | c4 b gis | a2
      } \break
      \repeat volta 2 {
        e'4 \sectionmark "2." | a2 g4 | f4 f e4 | d d c8 c | d2
        d8 d8 | d4 d e | c4. c8 c8 c8 | b4 a gis | a2
      }
      \repeat volta 2 {
        e'4 | a,2 b4 | c2 c4 | c2 e4 | d2
        d8 d8 | d4 d f | a2 e8 e | c4 b gis | a2
      } \break
      \repeat volta 2 {
        e'8 \sectionmark "3." e8 | a4 a g | f4 f4 e4 | d d c | d2
        d4 | d4 d e | c2 c8 c | b4 a gis | a2
      }
      \repeat volta 2 {
        e'4 | a, a b | c2 c4 | c c e | d4 d4
        d8 d8 | d4 d f | a2 e4 | c4 b gis | a4 a4
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1.A"
      \repeat volta 2 {
        Pe -- ço | for -- ça do | Céu, da | Ter -- ra~e do | Mar;
        Pe -- ço | for -- ça do | Ven -- to pa -- | ra me cu -- | rar.
      }
      \set stanza = "1.B"
      \repeat volta 2 {
        Eu | pe -- ço~a meu | Pai, ve -- | nha cla -- re -- | ar;
        Pe -- | ço~a mi -- nha | Mãe pra nos | i -- lu -- mi -- | nar.
      }
      \set stanza = "2.A"
      \repeat volta 2 {
        O | Sol, a | Lu -- a,~as Es -- | tre -- las a gui -- | ar;
        Pa -- ra | sem -- pre~es -- ta | Luz e -- ter -- na -- | men -- te~a bri -- | lhar.
      }
      \set stanza = "2.B"
      \repeat volta 2 {
        E -- | ter -- na -- | men -- te~a | nós gui -- | ar;
        Te -- nho | a mi -- nha | Mãe tam -- bem | te -- nho meu | Pai.
      }
      \set stanza = "3.A"
      \repeat volta 2 {
        Vou pe -- | din -- do~e ro -- | gan -- do~a to -- | dos meus ir -- | mãos;
        Va -- | mos com a -- | mor re -- ce -- | ber nos -- so | Pai.
      }
      \set stanza = "3.B"
      \repeat volta 2 {
        Va -- | mos se fir -- | mar nes -- | ta luz Di -- | vi -- na;
        Do nos -- | so Sal -- va -- | dor, de | quem nos en -- | si -- na.
      }
    }
    verseChords = \transpose a b \chordmode {
      \repeat volta 2 {
        s4 | a2.:m | f | a:7 | d:m
        | d:m | f | e:7 | a2:m
      }
      \repeat volta 2 {
        s4 | a2.:m | f | a:7 | d:m
        | d:m | f | e:7 | a2:m
      }
    }
    theChords = { \verseChords \verseChords \verseChords }
    %\setStaffSize #16 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{O poder do nosso Pai}[id={o-poder-do-nosso-pai-woueqr},by={Simja Johannes Lempinen},off={Otília},key={Dm},gk={}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/06-2p}
  \newchords{chords_opoderdo_a}\newchords{chords_opoderdo_b}
  \beginverse\memorize[chords_opoderdo_a]
    \beginrep
      Na|\[Dm]vego \[<*>]nestas |\[A]on\[<*>]-das \altchords{\id[1]{alt. (Dm)}|B&maj7 |E\textdegree (Gm6)}
      Que me |\[C]leva \[<*>]conhe|\[Dm]cer \[<*>]{} \altchords{|C |Dm}
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_opoderdo_b]
    \beginrep
      O po|\[Dm]der do \[<*>]nosso \[A]Pai \[<*>]{} \altchords{|F |A7}
      que faz |\[C]tudo \[<*>]renas|\[Dm]cer \[<*>]{} \altchords{|C |Dm}
    \endrep
  \endverse
  \beginverse\replay[chords_opoderdo_a]
    \beginrep
      Tenho |^força eu ^tenho |^tu^-do
      Tenho |^tudo que Ma^mãe me |^dá ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_opoderdo_b]
    \beginrep
      Para |^mim vi^ver na |^ter^-ra
      Com to|^da sa^tisfa|^ção ^
    \endrep
  \endverse
  \beginverse\replay[chords_opoderdo_a]
    \beginrep
      É ter fir|^meza e ^ter a|^mor ^
      Em nos|^so Pai ^Cria|^dor ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_opoderdo_b]
    \beginrep
      E em |^nossa ^Mãe Di|^vi^-na
      Sobe|^rana que ^nos en|^si^na
    \endrep
  \endverse
  \beginverse\replay[chords_opoderdo_a]
    \beginrep
      Tendo |^fé ^em si |^mes^-mo
      Em seu |^Eu Su^peri|^or ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_opoderdo_b]
    \beginrep
      Que po|^demos ^compreen|^der ^
      Que tudo |^temos ^que ven|^cer ^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative d'' {
      \key d \minor \time 4/4 \partial 4.
      \repeat volta 2 {
        r8\sectionmark "1." f4 | d4. d8 a4 f'4 | e2~e8 e8
        e8 f8 | g4. g8 f4 e4 | d2~d8
      } \break
      \repeat volta 2 {
        r8 d8 f8 | a4. a8 g4 f4 | e2.
        e8 f8 | g4. g8 f4 e4 | d2~d8
      } \break
      \repeat volta 2 {
        r8\sectionmark "2." f8 e8 | d4 d8 d8 a4 f'4 | e2~e8 e8
        e8 f8 | g8 g8 g8 g8 f4. e8 | d2~d8
      } \break
      \repeat volta 2 {
        r8 d8 f8 | a4 a4 g4 f4 | e2~e8 e8
        e8 f8 | g4. g8 f4 e4 | d2~d8
      } \break
      \repeat volta 2 {
        f8\sectionmark "3." f8 e8 | d4 d8 d8 a4 f'4 | e2.
        e8 f8 | g4. g8 f4 e4 | d2~d8
      } \break
      \repeat volta 2 {
        r8 d8 f8 | a4 a4 g4 f4 | e2~e8 e8
        e8 f8 | g4 g8 g8 f4 e4 | d2 d8
      } \break
      \repeat volta 2 {
        r8\posectionmark "4." f8 e8 | d2 a4 f'4 | e2~e8 e8
        e8 f8 | g4. g8 f4 e4 | d2~d8
      } \break
      \repeat volta 2 {
        r8 d8 f8 | a4. a8 g4 f8 f8 | e2~e8 e8
        e8 f8 | g4. g8 f4 e4 | d2~d8
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Na -- | ve -- go nes -- tas | on -- das;
        Que me | le -- va co -- nhe -- | cer.
        O po -- | der do nos -- so | Pai;
        que faz | tu -- do re -- nas -- | cer.
      \set stanza = "2."
        Te -- nho | for -- ça eu te -- nho | tu -- do;
        Te -- nho | tu -- do que Ma -- mãe me | dá.
        Pa -- ra | mim vi -- ver na | ter -- ra;
        Com to -- | da sa -- tis -- fa -- | ção.
      \set stanza = "3."
        É ter fir -- | me -- za e ter a -- | mor;
        Em nos -- | so Pai Cri -- a -- | dor.
        E em | nos -- sa Mãe Di -- | vi -- na;
        So -- be -- | ra -- na que nos en -- | si -- na.
      \set stanza = "4."
        Ten -- do | fé em si | mes -- mo;
        Em seu | Eu Su -- pe -- ri -- | or.
        Que po -- | de -- mos com -- pre -- en -- | der;
        Que tu -- do | te -- mos que ven -- | cer.
    }
    verseChords = \chordmode {
      \repeat volta 2 {
         s4. | d1:m | a
         | c | d2:m~8
      }
      \repeat volta 2 {
        s4. | d1:m | a
        | c | d2:m~8
      }
    }
    theChords = { \verseChords \verseChords \verseChords \verseChords }
    \setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Estou aqui}[id={estou-aqui-pka7f5},by={Simja Johannes Lempinen},off={Maria Serra},key={Dm},tags={mazurca}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/07-2e}
  \meter{6}{4}
  \beginverse\memorize
    \beginrep
      Estou a|\[Dm]qui estou a\[<*>]qui \altchords{\id[1]{alt. (Dm)}|B&maj7}
      Eu vim a|qui para di\[Gm6]zer \altchords{| - Gm6}
      É pre|\[A]ciso ter fir\[C]meza \altchords{|A C}
      No ca|\[A7]minho para ven\[Dm]cer \altchords{|A7 Dm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Estou |^firme eu estou ^firme
      Fir|mado no meu lu^gar
      Com a |^força do meu ^Pai
      Eu estou |pronto pra traba^lhar
    \endrep
  \endverse
  \beginverse
    \beginrep
      Estamos |^dentro da ba^talha
      Nao de|vemos esmore^cer
      Dei|^xando de pare^cer
      Que to|dos nos podemos ^ser
    \endrep
  \endverse
  \beginverse
    \beginrep
      A ba|^talha é pe^sada
      Ninguem |queira duvi^dar
      Que a |^Virgem Sobe^rana
      Es|tá a nos aju^dar
    \endrep
  \endverse
  \beginverse
    \beginrep
      Nos dá |^força nos dá co^ragem \altchords{\id[5]{L (Dm)}|Dm}
      No ca|minho para ven^cer \altchords{| - A}
      Com a |^força do meu ^Pai \altchords{|A7 A}
      Jesus |Cristo pra nos defen^der \altchords{|A7 Dm}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative d'' {
      \key d \minor \time 6/4 \partial 4.
      \repeat volta 2 {
        a8\sectionmark "1." a8 a8 | d4 d4 f8 e8 d4.
        d8 a8 a8 | d4. d8 f8 d8 e2
        e8 cis8 | a4 g'4 g8 f8 e4 e4
        e8 cis8 | a4 e'8 e8 e8 cis8 d4.
      }
      \repeat volta 2 {
        r8\sectionmark "2." a8 a8 | d4 d8 d8 f8 e8 d4 d4
        a4 | d4 d8 d8 f8 d8 e2
        e8 cis8 | a4 g'4 g8 f8 e4. e8
        e8 cis8 | a4 e'8 e8 e8 cis8 d4.
      }
      \repeat volta 2 {
        a8\sectionmark "3." a8 a8 | d4 d4 f8 e8 d4 d4
        a8 a8 | d4 d8 d8 f8 d8 e2
        e4 | a,4 g'8 g8 g8 f8 e2
        e8 cis8 | a4 e'8 e8 e8 cis8 d4.
      }
      \repeat volta 2 {
        r8\sectionmark "4." a8 a8 | d4 d4 f8 e8 d4 d4
        a8 a8 | d4 d4 f8 d8 e2
        e8 cis8 | a4 g'4 g8 f8 e4 e4
        e4 | a,4 e'8 e8 e8 cis8 d4.
      }
      \repeat volta 2 {
        r8\posectionmark "5." a8 a8 | d4 d8 d8 f8 e8 d4 d4
        a8 a8 | d4 d8 d8 f8 d8 e2
        e8 cis8 | a4 g'4 g8 f8 e2
        e8 e8 | a,8 a8 e'8 e8 e8 cis8 d4.
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Es -- tou a -- | qui es -- tou a -- qui;
        Eu vim a -- | qui pa -- ra di -- zer;
        É pre -- | ci -- so ter fir -- me -- za;
        No ca -- | mi -- nho pa -- ra ven -- cer.
      \set stanza = "2."
        Es -- tou | fir -- me eu es -- tou fir -- me;
        Fir -- | ma -- do no meu lu -- gar;
        Com a | for -- ça do meu Pai;
        Eu es -- tou | pron -- to pra tra -- ba -- lhar.
      \set stanza = "3."
        Es -- ta -- mos | den -- tro da ba -- ta -- lha;
        Nao de -- | ve -- mos es -- mo -- re -- cer;
        Dei -- | xan -- do de pa -- re -- cer;
        Que to -- | dos nos po -- de -- mos ser.
      \set stanza = "4."
        A ba -- | ta -- lha é pe -- sa -- da;
        Nin -- guem | quei -- ra du -- vi -- dar;
        Que a | Vir -- gem So -- be -- ra -- na;
        Es -- | tá a nos a -- ju -- dar.
      \set stanza = "5."
        Nos dá | for -- ça nos dá co -- ra -- gem;
        No ca -- | mi -- nho pa -- ra ven -- cer;
        Com a | for -- ça do meu Pai;
        Je -- sus | Cris -- to pra nos de -- fen -- der.
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4. | d1.:m | d2.:m g2.:m6
        | a2. c2. | a2.:7 d4.:m
      }
    }
    theChords = { \verseChords \verseChords \verseChords \verseChords \verseChords}
    %\setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Eu peço a meu Pai do Céu}[id={eu-peco-a-meu-pai-do-ceu-vv8p3b},by={Simja Johannes Lempinen},off={Lucas Serra},key={Bm},gk={}]
  \audio[key={Am}]{https://soundcloud.com/johannesbatista/08-2e}
  \newchords{chords_eupecoameupai_a}\newchords{chords_eupecoameupai_b}
  \transpose{2}
  \beginverse\memorize[chords_eupecoameupai_a]
    \beginrep
      Eu |\[Am]peço a \[<*>]meu Pai do |\[Dm]Céu\[<*>]{} \altchords{\id[1]{alt. (Bm)}|Bm |Em}
      E ao |\[E7]Meu Se\[<*>]nhor São Mi|\[Am]guel\[<*>]{} \altchords{|F#7 |Bm}
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_eupecoameupai_b]
    \beginrep
      Para |\[Am]vir \[<*>]nos aju|\[G]dar\[<*>]{} \altchords{|G |A}
      Este ter|\[E7]reiro \[<*>]ilumi|\[Am]nar\[<*>]{} \altchords{|F#7 |G}
    \endrep
  \endverse
  \beginverse\replay[chords_eupecoameupai_a]
    \beginrep
      Vamos |^todos ^nos se fir|^mar^ \altchords{\id[3]{(Am)}|Am |Dm}
      É no |^Céu na ^Terra e no |^Mar^ \altchords{|E7 |Am}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_eupecoameupai_b]
    \beginrep
      Para |^nos ^acredi|^tar^ \altchords{|Am |G}
      Se ali|^nhar, e ^se transfor|^mar^ \altchords{|E7 |Am}
    \endrep
  \endverse
  \beginverse\replay[chords_eupecoameupai_a]
    \beginrep
      Fir|^mado no ^Sol e na |^Lu^a \altchords{\id[5]{L (Am)}|Am |Dm}
      E nas Es|^trelas ^do firma|^men^to \altchords{|E |Am}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_eupecoameupai_b]
    \beginrep
      Vou se|^guir com ^meu São Mi|^guel^ \altchords{|C |Bm7&5}
      E corri|^gir o ^meu pensa|^men^to \altchords{|E7 |Am}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \transpose a b \relative e'' {
      \key a \minor \time 4/4 \partial 4
      \repeat volta 2 {
        e4\sectionmark "1." | a4. e8 g4 f8 e8 | d2.
        e8 f8 | e4. b8 d4 c8 b8 | a2.
      } \break
      \repeat volta 2 {
        a8 b8 | c2 e4 d8 c8 | b2~8
        e8 e8 f8 | e4. b8 d4 c8 b8 | a2.
      } \break
      \repeat volta 2 {
        e'8\sectionmark "2." e8 | a4. e8 g4 f8 e8 | d2.
        e8 f8 | e4. b8 d4 c8 b8 | a2.
      } \break
      \repeat volta 2 {
        a8 b8 | c2 e4 d8 c8 | b2~8
        e8 e8 f8 | e4. b8 d4 c8 b8 | a2.
      } \break
      \repeat volta 2 {
        e'4\sectionmark "3." | a4 a8 e8 g4 f8 e8 | d2 d8 d8
        e8 f8 | e4. b8 d4 c8 b8 | a2 a4
      } \break
      \repeat volta 2 {
        a8 b8 | c4. c8 e4 d8 c8 | b2~8
        e8 e8 f8 | e4. b8 d4 c8 b8 | a2 a4
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Eu | pe -- ço~a meu Pai do | Céu;
        E ao | Meu Se -- nhor São Mi -- | guel.
        Pa -- ra | vir nos a -- ju -- |dar;
        Es -- te ter -- | rei -- ro i -- lu -- mi -- nar.
      \set stanza = "2."
        Va -- mos | to -- dos nos se fir -- | mar;
        É no | Céu na Ter -- ra~e no | Mar;
        Pa -- ra | nos a -- cre -- di -- | tar;
        Se a -- li -- | nhar, e se trans -- for -- | mar.
      \set stanza = "3."
        Fir -- | ma -- do no Sol e na | Lu -- a;
        E nas Es -- | tre -- las do fir -- ma -- | men -- to;
        Vou se -- | guir com meu São Mi -- | guel;
        E cor -- ri -- | gir o meu pen -- sa -- | men -- to.
    }
    verseChords = \transpose a b \chordmode {
      \repeat volta 2 {
        s4 | a1:m | d1:m
        | e1:7 | a2.:m
      }
      \repeat volta 2 {
        s4 | a1:m | g1
        | e1:7 | a2.:m
      }
    }
    theChords = { \verseChords \verseChords \verseChords }
    %\setStaffSize #17 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Minha Mãe Minha Rainha}[id={minha-mae-minha-rainha-0761l8},by={Simja Johannes Lempinen},off={Sini},key={Dm},gk={}]
  \audio[key={Bm}]{https://soundcloud.com/johannesbatista/09-2m}
  \transpose{3}
  \beginverse\memorize
    \beginrep
      Minha |\[Bm]Mãe Mi\[F#7]nha Ra|\[Bm]i \[<*>]{-nha}
      Dâ li|\[Em6]cenca \[<*>]eu co|\[A]lher \[<*>]
      |Os vos\[<*>]sos re|\[D]médios
      \[<*>]Para |\[F#7]eu me \[<*>]conhe|\[Bm]cer \[<*>]
    \endrep
  \endverse
  \beginverse
    \beginrep
      Minha |^Mãe vós ^sois Ra|^i ^{-nha}
      Dona |^{do meu} ^cora|^ção ^
      Sois |quem me ^dá a |^luz
      ^No mun|^do de es^curi|^dão ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Ó |^Virgem ^Sobe|^ra ^{-na} \altchords{\id[3]{(Bm)}|Bm F#7 |Bm}
      Minha |^Mãe Pren^da Que|^ri^da \altchords{|Em6 |A}
      A|qui Vós ^agra|^deço \altchords{|A |D}
      Es^tou com |^Vós E^terna |^Vi^da \altchords{|F#7 |Bm}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Aju|^dai os ^vossos |^fi ^{-lhos} \altchords{\id[5]{L (Bm)}|Bm | \e}
      Neste |^mundo ^peca|^dor ^ \altchords{|A |(Em)}
      Para |todos ^ter a|^mor \altchords{|Em |F#}
      E a^presen|^tar o ^seu va|^lor ^ \altchords{| - |Bm}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \transpose b d' \relative e'' {
      \key b \minor \time 4/4 \partial 4
      \repeat volta 2 {
        b8\sectionmark "1." d8 | fis4 fis4 e4 fis4 | b,2~8 b8
        fis'8 fis8 | e4 d cis d | e1
        | e4 e d cis | fis4. fis8
        d4 b4 | e4 e4 d4 cis4 | b2.
      } \break
      \repeat volta 2 {
        b8\sectionmark "2." d8 | fis4 fis4 e4 fis4 | b,2~8 b8
        fis'8 fis8 | e4 d cis d | e2.
        e4 | e4 e d cis | fis2
        d4 b4 | e4 e4 d4 cis4 | b2.
      } \break
      \repeat volta 2 {
        b8( \sectionmark "3." d8) | fis4 fis4 e4 fis4 | b,2~8 b8
        fis'8 fis8 | e4 d cis d | e2 e8 r8
        e4 | e4 e d cis | fis4. fis8
        d4 b4 | e4 e4 d4 cis4 | b2 b8 r8
      } \break
      \repeat volta 2 {
        b8\sectionmark "4." d8 | fis4 fis4 e4 fis4 | b,2~8 b8
        fis'8 fis8 | e4 d cis d | e2.
        e8 e8 | e4 e d cis | fis4 fis4
        d4 b4 | e4 e4 d4 cis4 | b2.
      } \break
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Mi -- nha | Mãe Mi -- nha Ra -- | i -- nha;
        Dâ li -- | cen -- ca eu co -- | lher;
        | Os vos -- sos re -- | mé -- dios;
        Pa -- ra | eu me co -- nhe -- | cer.
      \set stanza = "2."
        Mi -- nha | Mãe vós sois Ra -- | i -- nha;
        Do -- na | do meu co -- ra -- | ção;
        Sois | quem me dá a | luz;
        No mun -- | do de~es -- cu -- ri -- | dão.
      \set stanza = "3."
        Ó | Vir -- gem So -- be -- | ra -- na;
        Mi -- nha | Mãe Pren -- da Que -- | ri -- da;
        A -- | qui Vós a -- gra -- | de -- ço;~Es --
        tou com | Vós E -- ter -- na | Vi -- da.
      \set stanza = "4."
        A -- ju -- | dai os vos -- sos | fi -- lhos;
        Nes -- te | mun -- do pe -- ca -- | dor;
        Pa -- ra | to -- dos ter a -- | mor;
        E~a -- pre -- sen -- | tar o seu va -- | lor.
    }
    verseChords = \transpose b d \chordmode {
      \repeat volta 2 {
        s4 | b2:m fis2:7 | b1:m
        | e1:m6 | a1
        | a1 | d1
        | fis1:7 | b2.:m
      }
    }
    theChords = { \verseChords \verseChords \verseChords  \verseChords }
    \setStaffSize #16 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Vivo aqui neste mundo pecador}[id={vivo-aqui-neste-mundo-pecador-1ickvq},by={Simja Johannes Lempinen},off={Milton},key={C},gk={}]
  \audio[key={C}]{https://soundcloud.com/johannesbatista/10-2v}
  \beginverse\memorize
    \beginrep
      Eu vivo a|\[C]qui neste \[F]mundo peca|\[C]dor \[<*>]{} \altchords{\id[1]{alt. (C)}|C F |C}
      Estou a|qui ensi\[F]nando meus ir|\[C]mão\[C7]s \altchords{| - F |C A7}
      Para |\[F]todos ter fir\[<*>]meza nesta |\[C]linha \altchords{|Dm7 G7 |Em7}
      E sa\[<*>]ber agrade|\[G7]cer os en\[<*>]sinos da Ra|\[C]i\[C7]nha \altchords{Am7 |Dm7 G7 |C C7}
      Para |\[F]todos ter fir\[<*>]meza nesta |\[C]linha \altchords{|Dm7 G7 |Em7}
      E sa\[<*>]ber agrade|\[G7]cer os en\[<*>]sinos da Ra|\[C]i\[<*>]nha \altchords{Am7 |Dm7 G7 |C}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Vou se|^guindo com ^fé em meu Se|^nhor ^ \altchords{\id[2]{alt. (C)}|C F |C}
      Vou rece|bendo esta ^luz com todo a|^mo^r \altchords{| - F |C C7}
      Peço |^força meu Ar^canjo São Mi|^guel \altchords{|F Fm |Em(7)}
      E as ^falanges do |^Céu confor^tai os filhos |^seu^s \altchords{Am7 |Dm7 G7 |C C7}
      Peço |^força meu Ar^canjo São Mi|^guel \altchords{|F Fm |Em(7)}
      E as ^falanges do |^Céu confor^tai os filhos |^seu^s \altchords{Am7 |Dm7 G7 |C}
    \endrep
  \endverse
  \beginverse % a verse just to display alt. chords
    \altchords{|\id[5]{L (C)}C G |C}
    \altchords{| - G |C C7}
    \altchords{|F |C}
    \altchords{|G7 |C C7}
    \altchords{|F |C}
    \altchords{|G7 |C}
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head-spacious.ly"
    theMelody = \relative c'' {
      \key c \major \time 4/4 \partial 4.
      \repeat volta 2 {
        g8\sectionmark "1.A" g8 g8 | c4 e8 e8 d8 d8 a8 b8 | c2~8
        g8 g8 g8 | c4 e8 e8 d8 d8 a8 b8 | c2~8 \break
        r8 c8\sectionmark "1.B:i" c8 | f8 f8 a8 a8 g8 g8 f8 f8 | e8 e8
        g8 g8 f8 f8 e8 e8 | d4 f8 f8 e8 e8 d8 d8 | c2 c8 \break
        r8 c8\sectionmark "1.B:ii" c8 | f8 f8 a8 a8 g8 g8 f8 f8 | e8 e8
        g8 g8 f8 f8 e8 e8 | d4 f8 f8 e8 e8 d8 d8 | c2 c8 \break
      }
      \repeat volta 2 {
        r8\sectionmark "2.A" g8 g8 | c4 e8 e8 d8 d8 a8 b8 | c2~8
        g8 g8 g8 | c8 c8 e8 e8 d8 d8 a8 b8 | c2~8 \break
        r8 c8\sectionmark "2.B:i" c8 | f8 f8 a8 a8 g8 g8 f8 f8 | e4
        g8 g8 f8 f8 e8 e8 | d4 f8 f8 e8 e8 d8 d8 | c2~c8 \break
        r8 c8\sectionmark "2.B:ii" c8 | f8 f8 a8 a8 g8 g8 f8 f8 | e4
        g8 g8 f8 f8 e8 e8 | d4 f8 f8 e8 e8 d8 d8 | c2~c8
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        Eu vi -- vo~a | qui nes -- te mun -- do pe -- ca -- | dor;
        Es -- tou a -- | qui en -- si -- nan -- do meus ir -- | mãos.
      \set stanza = "1.B"
        Pa -- ra | to -- dos ter fir -- me -- za nes -- ta | li -- nha;
        E sa -- ber a -- gra -- de -- | cer os en -- si -- nos da Ra -- | i -- nha.
        Pa -- ra | to -- dos ter fir -- me -- za nes -- ta | li -- nha;
        E sa -- ber a -- gra -- de -- | cer os en -- si -- nos da Ra -- | i -- nha.
      \set stanza = "2."
        Vou se -- | guin -- do com fé em meu Se -- | nhor;
        Vou re -- ce -- | ben -- do es -- ta luz com to -- do~a -- | mor.
      \set stanza = "2.B"
        Pe -- ço | for -- ça meu Ar -- can -- jo São Mi -- | guel;
        E as fa -- lan -- ges do | Céu con -- for -- tai os fi -- lhos | seus.
        Pe -- ço | for -- ça meu Ar -- can -- jo São Mi -- | guel;
        E as fa -- lan -- ges do | Céu con -- for -- tai os fi -- lhos | seus.
    }
    verseChords = \chordmode {
      \repeat volta 2 {
        s4. | c2 f2 | c1
        | c2 f2 | c2 c2:7
        | f1 | c1
        | g:7 | c2 c2:7
        | f1 | c1
        | g:7 | c2~8
      }
    }
    theChords = { \verseChords \verseChords }
    %\setStaffSize #16 % for better fit
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Divina Estrela}[id={divina-estrela-5ukygu},by={Simja Johannes Lempinen},off={Matti},key={Dm},gk={}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/11-2d}
  \newchords{chords_divinaestrela_a}\newchords{chords_divinaestrela_b}
  \beginverse\memorize[chords_divinaestrela_a]
    \beginrep
      Chamo |\[Dm]Eu, chamo |Eu
      Chamo |Eu Superi|\[A]or
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_divinaestrela_b]
    \beginrep
      Para |\[Gm]poder conhe|\[F]cer
      O meu |\[A]Mestre ensina|\[Dm]dor
    \endrep
  \endverse
  \beginverse\replay[chords_divinaestrela_a]
    \beginrep
      Aqui es|^tou, meu Pai man|dou
      O vosso |filho para ensi|^nar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_divinaestrela_b]
    \beginrep
      A to|^dos os meus Ir|^mãos
      Que qui|^serem acompa|^nhar
    \endrep
  \endverse
  \beginverse\replay[chords_divinaestrela_a]
    \beginrep
      Com meu |^Pai e minha |Mãe
      Divina Es|trela do meu cora|^ção
    \endrep
    \endverse\glueverses\beginverse\replay[chords_divinaestrela_b]
    \beginrep
      Vou para |^onde Deus qui|^ser
      Com a Vir|^gem da Concei|^ção
    \endrep
  \endverse
  \beginverse\replay[chords_divinaestrela_a]
    \beginrep
      Tendo |^fé, e tendo a|mor
      Que po|demos atraves|^sar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_divinaestrela_b]
    \beginrep
      Jesus |^Cristo é nosso Ir|^mão
      Ele |^vem acompa|^nhar
    \endrep
  \endverse
\endsong


\beginsong{Abençoai a minha vida}[id={abencoai-a-minha-vida-8cbi59},by={Simja Johannes Lempinen},off={Nandi},key={Dm},gk={},tags={valsa}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/12-2a}
  \newchords{chords_abencoai_a}\newchords{chords_abencoai_b}
  \meter{3}{4}
  \beginverse\memorize[chords_abencoai_a]
    Abenço|\[Dm]ai a |\[A]minha |\[Dm]vi|da
    Para |\[Gm]poder |rece|\[A]ber | \e
    \endverse\glueverses\beginverse\memorize[chords_abencoai_b]
    \beginrep
      A |\[Gm]Santissi|ma sa|\[Dm]úde | \e
      E eterna|\[F]mente a|\[A]grade|\[Dm]cer | \e
    \endrep
  \endverse
  \beginverse\replay[chords_abencoai_a]
    Agra|^deço ao |^Pai E|^ter|no
    E a Vir|^gem da |Concei|^ção | \e
    \endverse\glueverses\beginverse\replay[chords_abencoai_b]
    \beginrep
      Agra|^deço a |Jesus |^Cristo | \e
      Dentro |^do meu |^cora|^ção | \e
    \endrep
  \endverse
  \beginverse\replay[chords_abencoai_a]
    Dentro |^do meu |^cora|^ção | \e
    Encon|^trei um |Ser Di|^vi|no
    \endverse\glueverses\beginverse\replay[chords_abencoai_b]
    \beginrep
      Eu já |^me a|cho cu|^rado | \e
      Junto a|^qui com os |^meus Ir|^mãos | \e
    \endrep
  \endverse
  \beginverse\replay[chords_abencoai_a]
    Junto a|^qui com os |^meus Ir|^mãos | \e
    Dentro |^desta |primo|^zi|a
    \endverse\glueverses\beginverse\replay[chords_abencoai_b]
    \beginrep
      Dando |^viva ao |Pai E|^terno | \e
      E a Sem|^pre Vir|^gem Ma|^ri|a
    \endrep
  \endverse
\endsong


\beginsong{O Leão chegou}[id={o-leao-chegou-w9zvy7},by={Simja Johannes Lempinen},off={Aleksandr},key={E},gk={},tags={perfilado}]
  \audio[key={E}]{https://soundcloud.com/johannesbatista/13-2l}
  \newchords{chords_oleao_a}\newchords{chords_oleao_b}
  \beginverse\memorize[chords_oleao_a]
    \beginrep
      O Leão che|\[A]gou e confir|\[E]mou
      Sou |\[B]filho do Pai Cria|\[E]dor
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_oleao_b]
    \beginrep
      Sou |\[A]filho do meu |\[E]Pai
      a minha |\[B7]Mãe eu devo ter a|\[E]mor
    \endrep
  \endverse
  \beginverse\replay[chords_oleao_a]
    \beginrep
      O Leão che|^gou e abenço|^ou
      Todos |^nos devemos ter a|^mor
    \endrep
    \endverse\glueverses\beginverse\replay[chords_oleao_b]
    \beginrep
      Ter a|^mor a Todos |^Seres
      E a |^meu Divino Reden|^tor
    \endrep
  \endverse
  \beginverse\replay[chords_oleao_a]
    \beginrep
      O Leão che|^gou e perdo|^ou
      De todo |^mal ele me li|^vrou
    \endrep
    \endverse\glueverses\beginverse\replay[chords_oleao_b]
    \beginrep
      Me levan|^tou da Santa |^Cruz
      Com ale|^gria e com a|^mor
    \endrep
  \endverse
\endsong


\beginsong{Corneta de São Miguel}[id={corneta-de-sao-miguel-0wbg3n},by={Simja Johannes Lempinen},off={Helge},key={},gk={Dm},tags={valsa}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/14-2s}
  \meter{3}{4}
  \beginverse\memorize
    \beginrep
      A|\[Dm]qui mes|mo neste |mun|do
      De |grande |sofri|\[A]men|to
      Eu |vou can|tando e sor|rin|do
      Meus Ir|\[Gm]mãos vamos |\[A]ficar a|\[Dm]ten|tos
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meus Ir|^mãos vamos |ficar a|ten|tos
      |Nestas mu|danças de |^tem|po
      |Vamos to|dos se fir|mar | \e
      Com |^{o San}tíssi|^mo Sacra|^men|to
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu es|^tava na |praia do |mar | \e
      Ou|vindo a trom|beta to|^car | \e
      A cor|neta |de São Mi|guel | \e
      Que es|^tava a |^anunci|^ar | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Um se|^gredo pa|ra reve|lar | \e
      O Mis|tério |a desven|^dar | \e
      São |sete es|trelas no |Céu | \e
      Sete |^selos pa|^ra desa|^tar | \e
    \endrep
  \endverse
\endsong


\beginsong{Mistério da Santa Cruz}[id={misterio-da-santa-cruz-0vvegb},by={Simja Johannes Lempinen},off={Rosa},key={Bm},gk={},tags={valsa}]
  \audio[key={Bm}]{https://soundcloud.com/johannesbatista/15-2s}
  \meter{3}{4}
  \beginverse\memorize
    \beginrep
      Meus Ir|\[Bm]mãos aqui |eu vou lem|\[F#]brar | \e
      |\[A]Tudo que |já se pas|\[Em]sou | \e
      O mis|\[G]tério |da Santa |\[D]Cruz | \e
      O seu |\[F#]sangue por |nós derra|\[Bm]mou | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Ele so|^freu mais |tanto so|^freu | \e
      Que é |^para nós |também so|^frer | \e
      E atra|^ves des|te sofri|^men|to
      |^Tudo nós |poder ven|^cer | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Ele es|^{tá a} di|reita do |^Pai | \e
      Ensi|^nando a to|dos seus Ir|^mãos | \e
      Apren|^der a |se perdo|^ar | \e
      Para |^nosso Pai |nós perdo|^ar | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      O sofri|^mento é um |grande te|^sou|ro
      Que é |^para nós |bem apren|^der | \e
      Lapi|^dar o |nosso a|^mor | \e
      Para |^nosso Pai |nós rece|^ber | \e
    \endrep
  \endverse
\endsong


\beginsong{Príncipe Imperial}[id={principe-imperial-p84mbs},by={Simja Johannes Lempinen},off={Fabricia},key={Dm},gk={},tags={valsa}]
  \audio[key={Dm}]{https://soundcloud.com/johannesbatista/16-2p}
  \meter{3}{4}
  \beginverse\memorize
    \beginrep
      Meus Ir|\[Dm]mãos eu |vou avi|\[(F)]sar | \e
      Com as pa|\[Dm]lavras |deste can|\[A]tar | \e
      O |\[Gm]que vai |aconte|\[(B&)]cer | \e
      O mundo in|\[Gm]teiro vai |estreme|\[A]cer | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Quando |^sente a |força che|^gar | \e
      Tu te |^firma bem |em teu lu|^gar | \e
      A |^força |vem do as|^tral | \e
      Do |^Príncipe |Imperi|^al | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Vem |^seres da |terra e do |^mar | \e
      Da flo|^resta, do |vento e do |^ar | \e
      |^Todos vem |para lou|^var | \e
      O Cor|^deiro a|onde es|^tá | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meus Ir|^mãos prestem |bem aten|^ção | \e
      As pa|^lavras do |meu São Jo|^ão | \e
      É pre|^ciso |se arrepen|^der | \e
      E bons |^frutos ao |Pai ofere|^cer | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu |^sou Al|fa e O|^me|ga
      Eu |^sou prin|cípio e o |^fim | \e
      Quem |^não qui|ser respei|^tar | \e
      Vai |^ver o cor|po balan|^çar | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      O Le^|ão do Se|nhor já che|^gou | \e
      Ele |^veio pa|ra desa|^tar | \e
      Os |^selos do |livro da |^vi|da
      Meus Ir|^mãos vamos |se acor|^dar | \e
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meus Ir|^mãos vamos |se acor|^dar | \e
      O po|^der a|onde es|^tá | \e
      Es|^crito nu|ma pedra |^fi|na
      O nome a|^gora é |Jurami|^dam | \e
    \endrep
  \endverse
\endsong


\beginsong{Salmo}[id={salmo-l4a6j1},by={Simja Johannes Lempinen},off={Lea},key={C},gk={},tags={2x}]
  \audio[key={C}]{https://soundcloud.com/johannesbatista/17-2s}
  \newchords{chords_salmo_a}\newchords{chords_salmo_b}
  \beginverse\memorize[chords_salmo_a]
    \beginrep
      Meu |\[C]Pai Vós sois minha |\[G]Rocha
      Vós |\[Am]sois o meu re|\[C]fugio
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_salmo_b]
    \beginrep
      Em |\[Am]{ti eu} me se|\[C]guro
      Em |{ti eu} \[G]me re|\[C]novo
    \endrep
  \endverse
  \beginverse\replay[chords_salmo_a]
    \beginrep
      Dai-me |^força e dai-me a|^mor
      Meu Di|^vino Reden|^tor
    \endrep
    \endverse\glueverses\beginverse\replay[chords_salmo_b]
    \beginrep
      Vós |^sois o meu es|^cudo
      Vós |sois meu ^Profes|^sor
    \endrep
  \endverse
  \beginverse\replay[chords_salmo_a]
    \beginrep
      Treme a |^terra e geme o |^mar
      Tudo |^vai balance|^ar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_salmo_b]
    \beginrep
      Eu |^balanço nas |^ondas
      Si|go fe^liz can|^tando
    \endrep
  \endverse
\endsong


\beginsong{Santo Guerreiro}[id={santo-guerreiro-z0qo5f},by={Simja Johannes Lempinen},off={Jorma},key={C},gk={}]
  \audio[key={C}]{https://soundcloud.com/johannesbatista/18-2s}
  \newchords{chords_santoguerreiro_a}\newchords{chords_santoguerreiro_b}
  \beginverse\memorize[chords_santoguerreiro_a]
    \beginrep
      Lai |\[C]vem um Santo Guer|\[G]reiro
      Da |\[<G F#>Am6]corte de Je|\[C]sus
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_santoguerreiro_b]
    \beginrep
      Ele |\[Am]{é um} cava|\[C]leiro
      Prote|tor da \[G]Santa |\[C]Luz
    \endrep
  \endverse
  \notesoff
  \beginverse\replay[chords_santoguerreiro_a]
    \beginrep
      Lai |^vem o Santo Guer|^reiro
      Com sua |^espada na |^mão
    \endrep
    \endverse\glueverses\beginverse\replay[chords_santoguerreiro_b]
    \beginrep
      Ele |^traz a disci|^plina
      E bota |ordem ^no sa|^lão
    \endrep
  \endverse
  \beginverse\replay[chords_santoguerreiro_a]
    \beginrep
      Lai |^vem lai vem lai |^vem
      A |^{luz do} meu Je|^sus
    \endrep
    \endverse\glueverses\beginverse\replay[chords_santoguerreiro_b]
    \beginrep
      Para |^nos alumi|^ar
      Com a |vossa ^Santa |^Luz
    \endrep
  \endverse
  \beginverse\replay[chords_santoguerreiro_a]
    \beginrep
      Agra|^deço a meu Je|^sus
      Por |^tudo que vós me |^dá
    \endrep
    \endverse\glueverses\beginverse\replay[chords_santoguerreiro_b]
    \beginrep
      Eterna|^mente agrade|^cer
      E lou|vores ^eu vós |^dá
    \endrep
  \endverse
\endsong


\beginsong{A Disciplina}[id={a-disciplina-o7iosb},by={Simja Johannes Lempinen},off={Minna},key={Am},gk={}]
  \audio[key={Am}]{https://soundcloud.com/johannesbatista/19-2d}
  \newchords{chords_adisciplina_a}\newchords{chords_adisciplina_b}
  \beginverse\memorize[chords_adisciplina_a]
    Eu |\[Am]digo aos meus ir|\[E]mãos
    Vamos tra|\[C]tar de apren|\[E]der
    \endverse\glueverses\beginverse\memorize[chords_adisciplina_b]
    \beginrep
      Rece|\[Dm]ber o nosso |\[C]Mestre
      Aqui |\[\ac{1}{Bm7&5} \ac{2}{E7}]{dentro do Po}|\[\ac{1}{E} \ac{2}{Am}]der
    \endrep
  \endverse
  \beginverse\replay[chords_adisciplina_a]
    O |^Mestre vem che|^gando
    E faz |^tudo estreme|^cer
    \endverse\glueverses\beginverse\replay[chords_adisciplina_b]
    \beginrep
      Meus Ir|^mãos todos se |^firmam
      Nao |^{queiram esmore}|^cer
    \endrep
  \endverse
  \beginverse\replay[chords_adisciplina_a]
    O |^Mestre está a|^qui
    Ele é |^da Sobera|^nia
    \endverse\glueverses\beginverse\replay[chords_adisciplina_b]
    \beginrep
      Ele en|^sina ninguém faz |^caso
      Ninguem |^{trata de apren}|^der
    \endrep
  \endverse
  \beginverse\replay[chords_adisciplina_a]
    A disci|^plina vem che|^gando
    Das al|^turas do as|^tral
    \endverse\glueverses\beginverse\replay[chords_adisciplina_b]
    \beginrep
      Agora o |^relho está co|^mendo
      Quem qui|^{ser pode cho}|^rar
    \endrep
  \endverse
\endsong


\beginsong{Olhei para o firmamento}[id={olhei-para-o-firmamento-q1uzhj},by={Simja Johannes Lempinen},off={Jean-Michel},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/20-2s}
  \newchords{chords_olheipara_a}\newchords{chords_olheipara_b}
  \beginverse\memorize[chords_olheipara_a]
    \beginrep
      Olhei |\[D]para o firma|\[A]mento
      Vi u|\[<F#>D]ma es\[^<D>]tre\[<C#>F#]la \[^<C>]ca|\[<B>Bm]ir
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_olheipara_b]
    Caiu o |sol e cai\[F#]{u a} |\[Bm]lua
    Mas minha |fé em Vós \[F#]conti|\[Bm]nua
    Caiu o |\[(D)]sol e cai\[A]u a |\[D]lua
    Mas minha |fé em Vós \[A]conti|\[D]nua
  \endverse
  \notesoff
  \beginverse\replay[chords_olheipara_a]
    \beginrep
      Um novo |^céu uma nova |^terra
      Estava |^sendo a^nunci|^ado
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olheipara_b]
    Oh meu Di|vino ^Pai do |^Céu
    Espera |todos ^filhos |^seus
    Oh meu Di|^vino ^Pai do |^Céu
    Espera |todos ^filhos |^seus
  \endverse
  \beginverse\replay[chords_olheipara_a]
    \beginrep
      Escu|^tei um grande la|^mento
      Minha |^Mãe esta^va cho|^rando
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olheipara_b]
    Em |ver os ^vossos |^filhos
    Ilu|didos pela ^ilu|^são
    Em |^ver os ^vossos |^filhos
    Ilu|didos pela ^ilu|^são
  \endverse
  \beginverse\replay[chords_olheipara_a]
    \beginrep
      Escu|^tei um grande es|^trondo
      Foi meu |^Pai quem ^me ch|^amou
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olheipara_b]
    Eu |sigo ^com meu |^Mestre
    Je|sus para ^sempre a|^mém
    Eu |^sigo ^com meu |^Mestre
    Je|sus para ^sempre a|^mém
  \endverse
\endsong


\beginsong{Transmutação}[id={transmutacao-ee165d},by={Simja Johannes Lempinen},off={Linn},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/21-2t}
  \newchords{chords_transmutacao_a}\newchords{chords_transmutacao_b}
  \beginverse\memorize[chords_transmutacao_a]
    Eu |\[D]vim aqui neste |\[A]mundo
    |Com a minha caduca|\[D]ção
    \endverse\glueverses\beginverse\memorize[chords_transmutacao_b]
    \beginrep
      |\[D]Que me faz so|\[A]frer
      E alcan|çar a minha transforma|\[D]ção
    \endrep
  \endverse
  \beginverse\replay[chords_transmutacao_a]
    Alcan|^çando a minha transforma|^ção
    |Recebo este po|^der
    \endverse\glueverses\beginverse\replay[chords_transmutacao_b]
    \beginrep
      Que me |^faz compreen|^der
      Que |tudo eu hei de ven|^cer
    \endrep
  \endverse
  \beginverse\replay[chords_transmutacao_a]
    Jesus |^Cristo é o nosso |^Pai
    De |grande consola|^ção
    \endverse\glueverses\beginverse\replay[chords_transmutacao_b]
    \beginrep
      A|^onde ele pas|^sou
      |Eu tambem estou pas|^sando
    \endrep
  \endverse
  \beginverse\replay[chords_transmutacao_a]
    Mui|^tos querem ser |^grande
    Mas eu |fico bem pequeni|^ninho
    \endverse\glueverses\beginverse\replay[chords_transmutacao_b]
    \beginrep
      |^Piso bem ma|^cio
      |Junto com o meu Pa|^drinho
    \endrep
  \endverse
  \beginverse\replay[chords_transmutacao_a]
    O |^beija-flor pou|^sou
    |Dentro do meu cora|^ção
    \endverse\glueverses\beginverse\replay[chords_transmutacao_b]
    \beginrep
      Me entre|^gou esta linda |^flor
      A |cura da transmuta|^ção
    \endrep
  \endverse
\endsong


\beginsong{Espuma Branca}[id={espuma-branca-ghz7t7},by={Simja Johannes Lempinen},off={Alline},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/22a}
  \newchords{chords_espumabranca_a}\newchords{chords_espumabranca_b}
  \beginverse\memorize[chords_espumabranca_a]
    |\[D]Venho das ondas do |\[A]mar
    |Com meu alegre can|\[D]tar
    \endverse\glueverses\beginverse\memorize[chords_espumabranca_b]
    \beginrep
      Faço |tudo \[Bm]balan|\[A]çar
      E todos |Seres \[D]se ale|grar
    \endrep
  \endverse
  \beginverse\replay[chords_espumabranca_a]
    Nas minhas |^ondas tem Espuma |^Branca
    Eu vou cha|mando para lim|^par
    \endverse\glueverses\beginverse\replay[chords_espumabranca_b]
    \beginrep
      |Para pu^rifi|^car
      E todos |Filhos ^se ale|grar
    \endrep
  \endverse
  \beginverse\replay[chords_espumabranca_a]
    |^Sente na brisa do |^mar
    Suas |dores se alivi|^ar
    \endverse\glueverses\beginverse\replay[chords_espumabranca_b]
    \beginrep
      |Trago ^um con|^forto
      |Da Ma^mãe Ieman|já
    \endrep
  \endverse
\endsong


\beginsong{Amor}[id={amor-cdrge9},by={Simja Johannes Lempinen},off={Alexandre Borges},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/23-2a}
  \newchords{chords_amor_a}\newchords{chords_amor_b}
  \beginverse\memorize[chords_amor_a]
    \beginrep
      A|\[D]mor amor a|mor
      Do Nos|\[G]so Pai Cria|\[D]dor
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_amor_b]
    \beginrep
      A Sem|\[G]pre Virgem Ma|\[D]ria
      Ela en|\[A]sina \[G]todo |\[D]dia
    \endrep
  \endverse
  \beginverse\replay[chords_amor_a]
    \beginrep
      Ela en|^sina todo |dia
      Vem apren|^der a ter a|^mor
    \endrep
    \endverse\glueverses\beginverse\replay[chords_amor_b]
    \beginrep
      Ser fi|^el e obedi|^ente
      A Je|^sus Cris^to Reden|^tor
    \endrep
  \endverse
  \beginverse\replay[chords_amor_a]
    \beginrep
      Eu |^peço eu peço eu |peço
      A quem |^eu devo pe|^dir
    \endrep
    \endverse\glueverses\beginverse\replay[chords_amor_b]
    \beginrep
      Para nos |^ter nossa sa|^úde
      E nos vi|^ver em ^harmo|^nia
    \endrep
  \endverse
  \beginverse\replay[chords_amor_a]
    \beginrep
      Todo |^dia eu canto e |peço
      Minha |^Mãe Minha Ra|^inha
    \endrep
    \endverse\glueverses\beginverse\replay[chords_amor_b]
    \beginrep
      Que vós en|^trega as suas |^flores
      Aque|^les que ^mere|^cer
    \endrep
  \endverse
\endsong


\beginsong{Arcanjo Curador}[id={arcanjo-curador-gdm6yq},by={Simja Johannes Lempinen},off={Miguel},key={Am},gk={}]
  \audio[key={Am}]{https://soundcloud.com/johannesbatista/24-2a}
  \newchords{chords_arcanjocurador_a}\newchords{chords_arcanjocurador_b}
  \beginverse\memorize[chords_arcanjocurador_a]
    \beginrep
      Meu São Mi|\[Am]guel é Arcanjo Cura|dor
      Ele \[G]traz a leal|\[C]dade (de) Jesus \[Bm7&5]Cristo Reden|\[E]tor
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_arcanjocurador_b]
    Para sal|\[Dm]var os peca|\[Am]dores
    Estou a|\[Bm7&5]{qui para} traba|\[E]lhar
    Para sal|\[Dm]var os peca|\[Am]dores
    Estou a|\[E7]qui para traba|\[Am]lhar
  \endverse
  \beginverse\replay[chords_arcanjocurador_a]
    \beginrep
      Estou a|^qui para traba|lhar
      E aju^dar os meus Ir|^mãos po^{der se} levan|^tar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_arcanjocurador_b]
    Levan|^tar o pensa|^mento
    Ao |^{Deus Pai} Cria|^dor
    Levan|^{tar o} pensa|^mento
    Ao |^Deus Pai Cria|^dor
  \endverse
  \beginverse\replay[chords_arcanjocurador_a]
    \beginrep
      Esta |^é a Casa de |Deus
      Presta ^bem aten|^ção no que es^{tá aconte}|^cendo
    \endrep
    \endverse\glueverses\beginverse\replay[chords_arcanjocurador_b]
    Transmu|^tando o seu |^Ser
    Vai reve|^{lar Sou} eu sou |^eu
    Transmu|^tando o seu |^Ser
    Vai reve|^lar Sou eu sou |^eu
  \endverse
  \beginverse\replay[chords_arcanjocurador_a]
    \beginrep
      Sou |^eu Filho do Rei de A|mor
      Estou a^qui para mos|^trar a es^sencia do seu |^Ser
    \endrep
    \endverse\glueverses\beginverse\replay[chords_arcanjocurador_b]
    Vai se expan|^dindo no Infi|^nito
    Nave|^gando no As|^tral
    Seguindo |^sempre no ca|^minho
    Ao ru|^mo da perfei|^ção
  \endverse
\endsong


\beginsong{Meu caminho a seguir}[id={meu-caminho-a-seguir-cun4dy},by={Simja Johannes Lempinen},off={Flavio Speranza},key={Bm},gk={}]
  \audio[key={Bm}]{https://soundcloud.com/johannesbatista/25-2d}
  \newchords{chords_meucaminho_a}\newchords{chords_meucaminho_b}
  \beginverse\memorize[chords_meucaminho_a]
    \beginrep
      Vou destrin|\[Bm]chando meu caminho a se|\[A]guir
      Encostado a Minha |\[D]Mãe estou chegando no A|\[F#]lem
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_meucaminho_b]
    \beginrep
      Aonde |\[Em]tudo é Amor aonde |\[Bm]tudo é Certeza
      Que todos |\[F#]nós havemos de ven|\[\ac{2}{Bm}]cer
    \endrep
  \endverse
  \beginverse\replay[chords_meucaminho_a]
    \beginrep
      Que todos |^nós havemos de ven|^cer
      É com a força do meu |^Pai e Jesus Cristo Reden|^tor
    \endrep
    \endverse\glueverses\beginverse\replay[chords_meucaminho_b]
    \beginrep
      É quem me en|^sina a seguir nesta ba|^talha do Amor
      E para |^sempre louvar ao Senhor |^Deus
    \endrep
  \endverse
  \beginverse\replay[chords_meucaminho_a]
    \beginrep
      Para |^sempre louvar ao Senhor |^Deus
      Que é quem tem para nos |^dar O Alimento do E|^spírito
    \endrep
    \endverse\glueverses\beginverse\replay[chords_meucaminho_b]
    \beginrep
      Vamos re|^zar meus Irmãos pelo |^nosso Irmão
      Para |^ele poder se levan|^tar
    \endrep
  \endverse
  \beginverse\replay[chords_meucaminho_a]
    \beginrep
      Vou se|^guindo com o Rei do A|^mor
      Que ele é quem me en|^sina o rumo do bem vi|^ver
    \endrep
    \endverse\glueverses\beginverse\replay[chords_meucaminho_b]
    \beginrep
      Estou com |^Vós oh meu Pai Mais o |^Mestre Jesus
      E a Ra|^inha que está me ensi|^nando
    \endrep
  \endverse
\endsong


\beginsong{Reino da verdade}[id={reino-da-verdade-vtext6},by={Simja Johannes Lempinen},off={Sven},key={Bm},gk={},tags={valsa}]
  \audio[key={Bm}]{https://soundcloud.com/johannesbatista/26-1r}
  \newchords{chords_reinodaverdade_a}\newchords{chords_reinodaverdade_b}
  \meter{3}{4}
  \beginverse\memorize[chords_reinodaverdade_a]
    Pe|\[Bm]di a |minha es|\[F#]tre|la
    Que nos |\[Em]dê mais |calma e a|\[Bm]mor | \e
    \endverse\glueverses\beginverse\memorize[chords_reinodaverdade_b]
    \beginrep
      Para |\[Em]poder se|guir com ver|\[D]dade
      E |sinceri|\[\ac{1}{A} \ac{2}{F#}]dade,
      Fé em |nosso Se|\[\ac{1}{F#} \ac{2}{Bm}]nhor | \e
    \endrep
  \endverse
  \beginverse\replay[chords_reinodaverdade_a]
    A|^í meu |Pai respon|^deu | \e
    Vai se|^guindo pois |sois filho |^meu | \e
    \endverse\glueverses\beginverse\replay[chords_reinodaverdade_b]
    \beginrep
      Vai se|^guindo pela es|trada de a|^mor
      Que é |da Virgem |^Mãe
      E vosso |filho Reden|^tor | \e
    \endrep
  \endverse
  \beginverse\replay[chords_reinodaverdade_a]
    Eu es|^tava jun|tinho a Ma|^mãe | \e
    Sen|^tindo o a|mor de Pa|^pai | \e
    \endverse\glueverses\beginverse\replay[chords_reinodaverdade_b]
    \beginrep
      O vosso |^filho está |dentro de |^mim
      É quem |me faz sen|^tir
      Cho|rar e sor|^rir | \e
    \endrep
  \endverse
  \brk
  \beginverse\replay[chords_reinodaverdade_a]
    Meus ir|^mãos vim tra|zer esta |^pre|ce
    Quem qui|^ser vem |acompa|^nhar | \e
    \endverse\glueverses\beginverse\replay[chords_reinodaverdade_b]
    \beginrep
      Acompa|^nhar o do|no desta |^luz
      Que é |quem nos con|^duz
      Até os |pés de Je|^sus | \e
    \endrep
  \endverse
  \brk
  \beginverse\replay[chords_reinodaverdade_a]
    O |^Mestre sem|pre está pre|^sen|te
    Avi|^sando ao to|dos seus Ir|^mãos | \e
    \endverse\glueverses\beginverse\replay[chords_reinodaverdade_b]
    \beginrep
      Para en|^trar no rei|no da ver|^dade
      Só |com humil|^dade
      No |seu cora|^cão | \e
    \endrep
  \endverse
\endsong


\beginsong{Mãe das águas}[id={mae-das-aguas-3bwhur},by={Simja Johannes Lempinen},off={Farid},key={D},gk={}]
  \audio[key={D}]{https://soundcloud.com/johannesbatista/27-1m}
  \newchords{chords_maedasaguas_a}\newchords{chords_maedasaguas_b}
  \beginverse\memorize[chords_maedasaguas_a]
    \beginrep
      Oh! Mãe das |\[D]águas que cor\[G]re nas minhas |\[D]veias
      Oh! Mamãe |\[A]terra vós sois minha ma|\[D]téria
    \endrep
    \endverse\glueverses\beginverse\memorize[chords_maedasaguas_b]
    \beginrep
      Eu |\[A]vivo bem juntinho a Ma|\[D]mãe
      Ela |\[A]vive dentro do meu cora|\[D]cão
    \endrep
  \endverse
  \beginverse\replay[chords_maedasaguas_a]
    \beginrep
      Mamãe das |^matas ^que me ali|^menta
      E me con|^forta na vida espiritu|^al
    \endrep
    \endverse\glueverses\beginverse\replay[chords_maedasaguas_b]
    \beginrep
      Que me |^guia me dê paz e me dê |^luz
      Para se|^guir no caminho de Je|^sus
    \endrep
  \endverse
\endsong
