...
\beginsong{Maailma on kaunis}[by={Kalervo Halonen, Vexi Salmi}, key={Am}, gk={Am, (Am)--(Cm)}, ph={III}]
  \audio[key=Am]{https://soundcloud.com/larva-424852704/maailma-on-kaunis}
...
\beginsong{Tuuditan tulisoroista}[by={trad.}, key={Am}, gk={Gm, Gm--Bm}, tags={uni}]
  ...
  \mnbeginverse\memorize[chords_tulisoroinen_a]
    |\[\mnc{B}Em]Tuuditan \[^\mn{G}]tu|\[^\mn{F#}]li\[^\mn{B}]so\[^\mn{E}]rois\[^\mn{F#}]ta, |\[^\mn{G}]kipunaista \[\mnc{A}D]kiikut|\[\mnc{B}Em]telen \altchords{\id[1]{Em}|Em | - | - D |Em}
    \mnendverse\glueverses\mnbeginchorus\memorize[chords_tulisoroinen_b]
    |\[\mnc{B}Em]Vaa\[^\mn{E}]lin \[^\mn{B}]pien\[^\mn{A}]tä |\[^\mn{G}]val\[^\mn{B}]ke\[^\mn{E}]ais\[^\mn{F#}]ta, |\[\mnc{G}G]luojan lasta \[\mnc{F#}B7]liikut|\[\mnc{E}Em]telen \altchords{|Em | - |G B7 |Em}
  \mnendchorus
  \usealtmncolortrue% use alternate melody note color
  \beginverse% notes (\mn) in this verse signify the 2nd voice
    |\[\mnc{B}Em]Tai\[\mn{A}]va\[\mn{G}]hast' \[\mn{E}]on |lah\[\mn{D#}]ja \[\mn{E}]tul\[\mn{B}]lut, |\[\mn{E}]taivahan tu\[\mnc{D}D]len ki|\[\mnc{G}Em]soista
    \endverse\glueverses\beginchorus
    |\[\mnc{G}Em]Luojan lem\[\mn{F#}]men |\[\mn{E}]lei\[\mn{B}]kin\[\mn{E}]nöis\[\mn{D#}]tä, |\[\mnc{E}G]ei vahingon \[\mnc{F#}B7]val\[\mn{D#}]ke|\[\mnc{E}Em]oista
  \endchorus
  \notesoff
  ...
  \begin{lilywrap}\begin{lilypond}[]
    ...
    theChords = \transpose e a \chordmode {
      | e1:m | e:m | e2:m d | e1:m
      \repeat volta 2 {
        | e1:m | e:m | g2 b:7 | e1:m
      }
    }
    %\layout { #(layout-set-staff-size 15) } % for better fit
