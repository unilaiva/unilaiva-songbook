...
\beginsong{Aqui Estou Dizendo}[by={Mestre Irineu Serra},key={Am},gk={Am, Gm--Bm}]
  ...
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}\iflyriconly\scriptsize\fi % to fit whole song on one page in lyrics-only book
...
\beginsong{Marchinha \emph{(instrumental)}}[by={Mestre Irineu Serra},key={D \& C}, gk={D}]
  ...
  \ifchorded
    ...
    \beginchorus
      ...
      | |\[Em7]{} | |\[A] |\[D]{} |\[A] |\[D]{} |\[A7]{} |\[D]
    \endchorus
    \vspace{.6em} % for aligning with the next page
    \begin{lilywrap}\begin{lilypond}[] \include "tex/lp-include-head.ly"
      theMelody = \relative d'' {
        \key d \major \time 2/4
        \repeat volta 2 {
    ...
    \end{lilypond}\end{lilywrap}
    \vfill\vspace{6em}\brk % to push the rest onto the next page (or column)
    \musicnotefornext{(C):}
    \meter{2}{4}
    \beginchorus
      |\[C]{} | |\[G]{} |\[C]{} | |\[C7]{} | |\[F]
