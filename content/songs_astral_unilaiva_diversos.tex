...
\beginsong{Chamei Mamãe Jurema}[by={Valdete}, key={Bm}, gk={Cm, (Bm)--Cm--D\shrp{}m}]
  ...
  \mnbeginverse
    ...
    \beginrep \[^\mn{E}]Cha|\[Em]mei Papai \[\bmc\mn{G}]Ogum \[^\mn{F#}]da |\[\mnc{D}Bm]Ma\[^\mn{B}]ta\[\bm], \[^\mn{D}]cha|\[\mnc{C#}F#]mei Pa\[^\mn{D}]pai \[\mnc{E}F#7]O\[^\mn{D}]gum \[^\mn{C#}]do |\[\mnc{B}Bm]Mar \[\ac(1){B7}]{} \endrep
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    ...
    \beginrep Cha|^mei to^das as fa|^langes^, que Deus do|^mina na ^terra e no |^mar ^ \endrep
    \endrep
  \endverse
  \beginverse
    \beginrep
    Cha|^mei to^das essas |forças^, para |virem ^me aju|^dar ^
    \beginrep Me |^defender ^das tenta|^ções^, que |^so querem ^me derri|^bar ^ \endrep
    \endrep
  \endverse
  \beginverse
    \beginrep
    Tenho |^fé no ^meu Pai E|terno^, e na |minha Mãe ^Imacu|^la^da
    \beginrep Que |^com meu Se^nhor São Mi|^guel^, eu |^vou ven^cer a jor|^na^da \endrep
    \endrep
  \endverse
  \beginverse
    \beginrep
    É |^so pres^tar aten|ção^, na li|ção que es^ta estu|^dan^do
    \beginrep |^Para ^poder pas|^sar^, nas |^provas ^que vem che|^gan^do \endrep
    \endrep
  \endverse
  \beginverse
    \beginrep\ifshowlilypond\normalsize\fi % to fit on a spread
    Es|^tuda ^com aten|ção^, na es|cola que se ^matricu|^lou ^
    \beginrep Para pro|^var que ^é bom a|^luno^: e es|^ta com o ^seu Profes|^sor ^ \endrep
    \endrep
  \endverse
  \beginverse
    \beginrep\ifshowlilypond\normalsize\fi % to fit on a spread
    |^Eu pro^meto a meu |Mestre^, que eu |vou pi^sar no ba|^ten^te
    \beginrep |^Que de a^gora pra |^frente, e ^olho por |^olho e ^dente por |^den^te \endrep
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
...
\beginsong{Coração Alado}[by={Regina Pereira}, key={Dm}, gk={Dm, Cm--E\flt{}m}, tags={valsa}]
...
\beginsong{Mensageiro das Estrelas}[by={Vitor Vitti}, key={Dm}, gk={Dm, Cm--E\flt{}m}]
