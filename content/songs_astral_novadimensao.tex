...
\beginsong{Renascer}[by={Padr. Alfredo},key={C},gk={C, G\shrp{}--B--(C)}]
  ...
  \newchords{chords_renascer_a}\newchords{chords_renascer_b}
  \mnbeginverse\memorize[chords_renascer_a]
    ...
    \mnendverse\glueverses\mnbeginchorus\memorize[chords_renascer_b]
    \[^\bmc\mnvii{E}{C}]Aqui |\[\mnc{C}C]na nos\[\bm]sa mis|\[\mnc{F}F]são o \[\bm]nosso |\[\mnc{A}Am]Mestre \[\bm]Re\[^\mn{G}]plan|\[\mnc{C}C]tou
    \[\bmc\mn{G}]Doutri|\[\mnc{A}G]na \[^\mn{G}]de \[\bm]Jesus |\[\mnc{D}Dm]Cristo \[\bm]do nos|\[\mnc{G}G]so Pai \[\bmc\mn{D}]Cri\[^\mn{E}]a|\[\mnc{C}C]dor.
  \mnendchorus
  \notesoff
  \beginverse\replay[chords_renascer_a]
    É |^nos^sa Es|cola, ^é o |^nosso ^Profes|^sor ^
    O |nosso ^Tempo\jw é |^este, ^vamos |^todos ^dar lou|^vor \[\bm]
    \endverse\glueverses\beginchorus\replay[chords_renascer_b]
