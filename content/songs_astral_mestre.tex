...
\beginsong{Papai Paxá}[by={Mestre Irineu Serra},key={B dor.},gk={B dor., (B dor.) -- (C dor.)}]
  ...
  \mnbeginchorus\memorize[chords_papaipaxa_a]
    ...
    \[^\mn{D}]Eu |\[^\mn{F#}]vim bei\[\bmc\mn{G#}]ran\[^\mn{A}]{do o} |\[\mnc{B}Bm]mar \[\bm] \altchords{| - |Bm}
  \mnendchorus
  \notesoff
...
\beginsong{Dois de Novembro}[by={Mestre Irineu Serra},key={Gm},gk={Gm, (Gm)--(G\shrp{}m)}]
  ...
  \endchorus
  \beginchorus
    |\[Gm]Aqui a|^chou, a|qui dei|^xou | \e
...
\beginsong{A Rainha Me Mandou}[by={Mestre Irineu Serra},tags={valsa},key={C},gk={C, G--E}]
  ...
  \begin{lilywrap}\begin{lilypond}[]
    ...
    % % transcribed by larva, latest update on 2024-03
    % \header {
    %   title = "78. A Rainha Me Mandou"
    %   composer = "Mestre Raimundo Irineu Serra"
    % }
    theMelody = \relative d'' {
      \key c \major \slurDashed % = a major = b dorian
      \set melismaBusyProperties = #'()
      \time 3/4 \partial 2
      \sectionLabel "A"
      \repeat volta 2 {
        c4 e | g2 e4 | a2 e4 | g2.~ | g4
