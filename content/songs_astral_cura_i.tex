% Santo Daime: Cura I
% ===================
%
\audio[title={Nossa Irmandade}]{https://www.nossairmandade.com/hinario/2/CuraI}

% The following sets the song number for the first song in this file.
% The number will automatically be incremented by one for each song.
% Please do not change this! Changing would make different versions of
% the songbook to have different numbers for the same songs, and it
% would totally mess up the selection booklets causing them to have
% wrong songs in them. (For the same reason, add new songs only to the
% end of each songs_ file.)
\setcounter{songnum}{1}


\beginsong{Pai Nosso}[id={pai-nosso-z9d35r},by={Sebastião},key={C},gk={D, B&--G}]
  \audio{https://www.nossairmandade.com/hymn/53/PaiNosso}
  \newchords{chords_painosso_a}\newchords{chords_painosso_b}
  \mnbeginverse
    \beginrep\memorize[chords_painosso_a]
      \[^<A>]Pai |\[<C>C]Nosso que\jw es\[<* D>]tá \[^<C>]no |\[<A>Am]céu \altchords{\id[1]{(D)}|D |Bm}
      San\[<* C>]ti\[^<E>]fi|\[<C>C]cado\jw é \[^<B>]nos\[<A>Am]so \[^<C>]Se|\[<E>Em]nhor\[<*>]{} \altchords{|D Bm |F#m}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_painosso_b]
      \[^<D>]Aprovei|\[Dm]tando\jw a o\[<*>]ca\[^<C>]si|\[<A>Am]ão \altchords{|Em |Bm}
      Meu \[<* C>]Mes\[^<E>]tre\jw é |\[<C>C]Rei s\[^<B>]u\[<A>Am]pe\[^<C>]ri|\[C]or \[<*>]{} \altchords{|D Bm |D}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_painosso_a]
    \beginrep
      Oh |^minha ^Virgem |^Mãe
      As ^suas |^lágrimas ^derra|^mou ^
    \endrep
    \endverse\glueverses\beginverse
    \beginrep\replay[chords_painosso_b]
      A|^queles ^que não |^amam
      ^Vão sen|^tir a ^mesma |^dor ^
    \endrep
  \endverse
  \beginverse\replay[chords_painosso_a]
    \beginrep
      Meu |^Mestre es^tá co|^migo
      ^Ele\jw é |^Mestre en^sina|^dor ^
    \endrep
    \endverse\glueverses\beginverse
    \beginrep\replay[chords_painosso_b]
      Jun|^{to com} a ^Virgem |^Mãe
      O ^nosso |^Pai foi ^quem man|^dou ^
    \endrep
  \endverse
  \beginverse\replay[chords_painosso_a]
    \beginrep
      Nin|^guém sa^be\jw o que |^diz
      E ^nem sa|^bem o que\jw es^tão di|^zen^do
    \endrep
    \endverse\glueverses\beginverse
    \beginrep\replay[chords_painosso_b]
      Com o po|^der do ^nosso |^Pai
      O ^relho\jw a|^gora es^{tá co}|^men^do
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Pai Nosso"
    %   composer = "Sebastião"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative a' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 4.
      \sectionLabel "A"
      \repeat volta 2 {
        r8\pomark a4 | c4 c8( c8) d4 c | a4. a8 c4 e | c c8 b a4 c | e2~ e8
      }
      \sectionLabel "B"
      \repeat volta 2 {
        \parenthesize d8 d( d) | d4 d8( d8) d4 c4 | a4. a8 c4 e | c4 c8( b8) a4 c4 | c2~ c8
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      Pai | Nos -- so que~es -- tá no | céu;
      San -- ti -- fi -- | ca -- do~é nos -- so Se -- | nhor. __ _
      \set stanza = "1.B"
      A -- pro -- vei -- | tan -- do~a o -- ca -- si -- | ão;
      Meu Mes -- tre~é | Rei su -- _ pe -- ri -- | or. __ _
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      Oh | mi -- nha __ _ Vir -- gem | Mãe;
      As su -- as | lá -- gri -- mas der -- ra -- | mou. __ _
      \set stanza = "2.B"
      "" A -- _ | que -- les __ _ que não | a -- mam;
      Vão sen -- | tir a __ _ mes -- ma | dor. __ _
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      Meu | Mes -- tre es -- tá co -- | mi -- go;
      E -- le~é | Mes -- tre en -- si -- na -- | dor. __ _
      \set stanza = "3.B"
      "" Jun -- _ | to com a Vir -- gem | Mãe;
      O nos -- so | Pai foi __ _ quem man -- | dou. __ _
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      Nin -- | guém sa -- _ be~o que | diz;
      E nem sa -- | bem o que~es -- tão di -- | zen -- do.
      \set stanza = "4.B"
      Com o po -- | der do __ _ nos -- so | Pai;
      O re -- lho~a -- | go -- ra es -- tá co -- | men -- do.
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4. | c1 | a:m | c2 a2:m | e2:m~ e8:m
      }
      \repeat volta 2 {
        s4. | d1:m | a:m | c2 a:m | c2~ c8
      }
    }
    \setStaffSize #15 % for better division
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]
    Our father who art in heaven, hallowed is our Lord
    Using the occasion, my Master is superior King
    \nextverse
    Oh! My Virgin Mother, your tears were shed
    Those who do not love are going to feel the same pain
    \nextverse
    My Master is with me, he is the Master teacher
    Together with the Virgin Mother, it was ordered by our father
    \nextverse
    No one knows what they say, nor do they know what they're saying
    With the power of our Father, now the whip is in full swing
  \end{translation}
\endsong


\beginsong{Eu Chamei Meu Mestre}[id={eu-chamei-meu-mestre-9ajp1f},by={Sebastião},key={C},gk={D, C--E}]
  \audio{https://www.nossairmandade.com/hymn/54/EuChameiMeuMestre}
  \newchords{chords_euchameimeumestre_a}\newchords{chords_euchameimeumestre_b}
  \mnbeginverse
    \beginrep\memorize[chords_euchameimeumestre_a]
      |\[<C>C]Eu \[^<A>]cha\[<D>G]mei \[^<C>]meu |\[C]Mes\[<*>]tre \[^<G>]em |\[<E C>Am]no\[^<E>]me \[<D>Dm]da \[^<C>]ver|\[<E>Em]da\[<*>]de
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_euchameimeumestre_b]
      \[^<G>]Que |\[<E>Am]a \[^<C>]ver\[<D>Dm]da\[^<C>]de\jw é |\[<G>G]Deus\[<*>], \[^<E>]o |\[<C>C]rei \[^<A>]da \[<D>G]ca\[^<C>]ri|\[C]da\[<*>]de
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      |^Eu pe^ço\jw a meu |^Mes^tre de |^todo meu ^cora|^ção ^ \altchords{\id[1]{(D)}|D A |D |Bm Em |F#m}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Meu |^Mestre tenha ^pie|^da^de, |^abra nos^sa ses|^são ^ \altchords{|Bm Em |A |D A |D}
    \endrep
  \endverse
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      As|^sim está a^conte|^cen^do, dei|^xa ficar ^como\jw es|^tá ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Que é |^para ^conhe|^ce^rem, sa|^berem me ^respei|^tar ^
    \endrep
  \endverse
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      Já |^disse e tor^no\jw a di|^zer^, não |^querem a^credi|^tar ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Le|^vanto\jw a mi^nha ses|^são ^ para\jw o |^reino ce^lesti|^al ^
    \endrep
  \endverse
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      Meus ir|^mãos va^mos o|^rar^, ro|^gar e va^mos pe|^dir ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Para le|^varmos es^ta ver|^da^de que o |^Mestre dei^xou a|^qui ^
    \endrep
  \endverse
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      Todos |^querem ^ser fe|^liz^, cada |^um só ^quer pra |^si ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Di|^zendo que es^tão co|^mi^go, es|^tão lon^ge de |^mim ^
    \endrep
  \endverse
  \beginverse\replay[chords_euchameimeumestre_a]
    \beginrep
      Eu |^não dei^xei as|^sim^, dei|^xei foi pa^{ra\jw ajun}|^tar ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euchameimeumestre_b]
    \beginrep
      Não fi|^zeram ^como\jw eu |^dis^se, fi|^zeram foi ^espa|^lhar ^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Eu Chamei Meu Mestre"
    %   composer = "Sebastião"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative e'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 4
      \sectionLabel "A"
      \repeat volta 2 {
        \parenthesize e8( \pomark \parenthesize e8) | c8( c c) a8 d4. c8 | c2( c4) g'8( g) | e8( c4) e8 d4. c8 | e2( e4)
      }
      \sectionLabel "B"
      \repeat volta 2 {
        \grace{\altcol\parenthesize g8\altmark "5." } g8( g) | e8( e e) c d4. c8 | g2( g4) e'8( e) | c4( c8) a8 d4. c8 | c2( c4)
      }
    }
    theLyricsOne = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "1."
      \repeat volta 2 {
        \skip 1 \skip 1 | Eu __ _ _ cha -- mei meu | Mes -- tre
        em __ _ | no -- _ me da ver -- | da -- de.
      }
      \set stanza = "1.B"
      \repeat volta 2 {
        \skip 1 Que __ _ | a __ _ _ ver -- da -- de~é | Deus, __ _
        o __ _ | rei __ _ da ca -- ri -- | da -- de.
      }
    }
    theLyricsTwo = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "2."
      \repeat volta 2 {
        \skip 1 \skip 1 | Eu __ _ _ pe -- ço~a meu | Mes -- tre
        de __ _ | to -- do meu co -- ra -- | ção. __ _
      }
      \set stanza = "2.B"
      \repeat volta 2 {
        \skip 1 Meu __ _ | Mes -- tre te -- nha pi -- e -- | da -- de, \skip 1 \skip 1
        | a -- bra nos -- sa ses -- | são. __ _
      }
    }
    theLyricsThree = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "3."
      \repeat volta 2 {
        As -- _ | sim es -- tá a -- con -- te -- | cen -- do,
        dei -- _ | xa fi -- car co -- mo~es -- | tá. __ _
      }
      \set stanza = "3.B"
      \repeat volta 2 {
        \skip 1 Que é | pa -- _ _ ra co -- nhe -- | ce -- rem,
        sa -- _ | be -- rem me res -- pei -- | tar. __ _
      }
    }
    theLyricsFour = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "4."
      \repeat volta 2 {
        Já __ _ | dis -- se e tor -- no~a di -- | zer, __ _
        não __ _ | que -- rem a -- cre -- di -- | tar. __ _
      }
      \set stanza = "4.B"
      \repeat volta 2 {
        \skip 1 Le -- _ | van -- _ to~a mi -- nha ses -- | são __ _
        pa -- ra~o | rei -- no ce -- les -- ti -- | al. __ _
      }
    }
    theLyricsFive = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "5."
      \repeat volta 2 {
        Meus ir -- | mã -- _ os va -- mos o -- | rar, __ _
        ro -- _ | gar e va -- mos pe -- | dir. __ _
      }
      \set stanza = "5.B"
      \repeat volta 2 {
        \altcol Pa -- ra le -- | var -- _ mos es -- ta ver -- | da -- de
        que o | Mes -- tre dei -- xou a -- | qui. __ _
      }
    }
    theLyricsSix = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "6."
      \repeat volta 2 {
        To -- dos | que -- _ _ rem ser fe -- | liz, __ _
        ca -- da | um __ _ só quer pra | si. __ _
      }
      \set stanza = "6.B"
      \repeat volta 2 {
        \skip 1 Di -- _ | zen -- do que es -- tão co -- | mi -- go,
        es -- _ | tão __ _ lon -- ge de | mim. __ _
      }
    }
    theLyricsSeven = \lyricmode {
      \set includeGraceNotes = ##t
      \set stanza = "7."
      \repeat volta 2 {
        Eu __ _ | não __ _ _ dei -- xei as -- | sim, __ _
        dei -- _ | xei foi pa -- ra~a -- jun -- | tar. __ _
      }
      \set stanza = "7.B"
      \repeat volta 2 {
        \skip 1 Não fi -- | ze -- _ _ ram co -- mo~eu | dis -- se,
        fi -- _ | ze -- ram foi es -- pa -- | lhar. __ _
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4 | c2 g | c1
        | a2:m d:m | e2.:m
      }
      \repeat volta 2 {
        \grace s8 s4 | a2:m d:m | g1
        | c2 g | c2.
      }
    }
    \setStaffSize #15 % for better division
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]\iflyriconly\scriptsize\fi% to fit on one page in lyrics-only books
    I called my Master in the name of the truth
    Because the truth is God, the King of charity
    \nextverse
    I ask my Master with all of my heart
    My Master have mercy, open our session
    \nextverse
    This is how it's happening, leave it as it is
    So that all know, know how to respect me
    \nextverse
    I have said and I say it again: you don't want to believe
    I raise up my session to the Celestial Kingdom
    \nextverse
    My brothers and sisters, let's pray; implore, and let's ask
    For us to take this truth that the Master left here
    \nextverse
    Everyone wants to be happy, each one only wants this for oneself
    Saying that they are with me they are far from me
    \nextverse
    I didn't leave things this way, I left things so they could gather.
    They didn't do as I said, what they did was scatter.
  \end{translation}
\endsong


\beginsong{Chamei o Mestre Juramidam}[id={chamei-o-mestre-juramidam-ehyuzi},by={Sebastião},key={C},gk={D, C--E}]
  \audio{https://www.nossairmandade.com/hymn/55/ChameiOMestreJuramidam}
  \mnbeginverse
    \beginrep\memorize
      \[^<G>]Chamei o |\[<C>C]Mes\[<* G>]tre \[^<C>]Jurami|\[<E>Am]dam\[<*>]{} \altchords{\id[1]{(D)}|D |Bm}
      \[^<C>]Pa\[^<E>]ra\jw o |\[<G>G]meu \[^<C>]Mes\[<E>Em]tre \[^<G>]vir |\[<C>C]cá \[<*>]{} \altchords{|A F#m |D}
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    \beginrep
      Com su|^a ^ força di|^{vi -} ^ na
      Para |^vir me ^liber|^tar^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Dai-me |^for^ça, dai-me\jw a|^mor^ \altchords{\id[2]{(E)}|E |C#m}
      Para\jw eu |^poder ^domi|^nar^ \altchords{|B G#m |E}
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meu |^Mes^tre em sua |^{cor -} ^ te
      Em seu |^trono ^divi|^nal^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Tudo |^tem^, tudo |^dá ^
      Com su|^a luz ^de cris|^tal^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Sou pe|^que^no, sou pe|^{que -} ^ no
      É a |^quem\jw eu de^vo pe|^dir^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Todos |^têm^, todos |^têm^
      Todos |^têm, po^dem se|^guir^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Aqui |^eu ^ vou decla|^rar ^
      A quem o |^Mestre ^entre|^gou^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Quem |^a^ma e sabe\jw a|^mar^
      Segue |^na\jw estrada ^do a|^mor^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meu |^Mes^tre me de|^{cla -} ^ re
      Como |^é que ^vai fi|^car^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Preste\jw aten|^ção ^ no que\jw eu já |^{dis -} ^ se
      Quem ven|^cer\jw é que ^vai bri|^lhar^
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Chamei o Mestre Juramidam"
    %   composer = "Sebastião"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative g' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 4.
      \repeat volta 2 {
        \parenthesize g8\pomark g g | c2( g8) \parenthesize c c c | \once\slurSolid e2( e8) \parenthesize c8 c8 e8 | g4. c,8 e4. g,8 | c2~ c8
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        Cha -- mei o | Mes -- tre Ju -- ra -- mi -- | dam; _
        "" Pa -- ra~o | meu Mes -- tre vir | cá. _
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      \repeat volta 2 {
        \skip 1 Com su -- | a __ _ for -- ça di -- | vi _ -- na;
        Pa -- ra | vir me li -- ber -- | tar. _
      }
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      \repeat volta 2 {
        \skip 1 Dai -- me | for -- ça, \skip 1 dai -- me~a -- | mor; _
        "" Pa -- ra~eu | po -- der do -- mi -- | nar. _
      }
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      \repeat volta 2 {
        \skip 1 Meu __ _ | Mes -- tre em su -- a | cor _ -- te;
        Em seu | tro -- no di -- vi -- | nal. _
      }
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5."
      \repeat volta 2 {
        \skip 1 Tu -- do | tem, __ _ "" tu -- do | dá; _
        "" Com su -- | a luz de cris -- | tal. _
      }
    }
    theLyricsSix = \lyricmode {
      \set stanza = "6."
      \repeat volta 2 {
        \skip 1 Sou pe -- | que -- no, \skip 1 sou pe -- | que _ -- no;
        É a | quem~eu de -- vo pe -- | dir. _
      }
    }
    theLyricsSeven = \lyricmode {
      \set stanza = "7."
      \repeat volta 2 {
        \skip 1 To -- dos | têm, __ _ "" to -- dos | têm; _
        "" To -- dos | têm, po -- dem se -- | guir. _
      }
    }
    theLyricsEight = \lyricmode {
      \set stanza = "8."
      \set includeGraceNotes = ##t
      \repeat volta 2 {
        \skip 1 A -- qui | eu __ _ vou de -- cla -- | rar; _
        A quem o | Mes -- tre en -- tre -- | gou. _
      }
      \set includeGraceNotes = ##f
    }
    theLyricsNine = \lyricmode {
      \set stanza = "9."
      \repeat volta 2 {
        \skip 1 Quem __ _ | a -- ma e sa -- be~a -- | mar; _
        "" Se -- gue | na~estra -- da do a -- | mor. _
      }
    }
    theLyricsTen = \lyricmode {
      \set stanza = "10."
      \repeat volta 2 {
        \skip 1 Meu __ _ | Mes -- tre \skip 1 me de -- | cla _ -- re;
        Co -- mo | é que vai fi -- | car. _
      }
    }
    theLyricsEleven = \lyricmode {
      \set stanza = "11."
      \repeat volta 2 {
        Pres -- te~a -- ten -- | ção __ _ no que~eu já | dis _ -- se;
        Quem ven -- | cer~é que vai bri -- | lhar. _
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4. | c1 | a:m | g2 e:m | c2~ c8
      }
    }
    \setStaffSize #14 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}[EN]
    I called the Master Juramidam, for my Master to come here
    \nextverse
    With his divine force to come and liberate me
    \nextverse
    Give me strength, give me love, for me to be able to dominate
    \nextverse
    My Master in His court, on His divine throne
    \nextverse
    He has and gives everything with His crystal light
    \nextverse
    I am small, I am small, it's he whom I should ask
    \nextverse
    Everyone has, everyone has, everyone has, you can follow
    \nextverse
    Here I'm going to declare to whom the Master entrusted it
    \nextverse
    He who loves and knows how to love, follows the path of love
    \nextverse
    My Master declares to me how things are going to be
    \nextverse
    Pay attention to what I've already said; he who overcomes is going to shine
  \end{translation}
\endsong


\beginsong{As Estrelas}[id={as-estrelas-pardj6},by={Mestre Irineu Serra},key={A},gk={A, G--B}]
  \audio{https://www.nossairmandade.com/hymn/265/AsEstrelas}
  \mnbeginverse
    \beginrep\memorize
      \[^<C#>]As \[^<E>]es|\[<A>A]trelas \[<* C#>]já \[^<E>]che|\[^<A>]garam \altchords{\id[1]{(G)}|G | \e}
      \[<* A>]Para \[^<G#>]di|\[<F#>F#m]zer \[^<E>]o \[(A)]no\[^<C#>]me |\[<E>E]seu \[<*>]{} \altchords{|Em (G) |D}
      \[^<E>]Sou |\[<A>A]eu, sou \[<* C#>]eu, \[^<E>]sou |\[(E)]eu \altchords{|G |(D)}
      \[<A>A]Sou \[^<G#>]eu |\[<F#>F#m]um \[^<E>]fi\[E]lho \[^<C#>]de |\[<D>D]Deus \[<*>]{} \altchords{G |Em D |C}
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    \beginrep
      As es|^trelas ^me le|varam
      ^Para cor|^rer o ^mundo\jw in|^tei^ro
      \noteson\[<C#>]Pra co\[<E>]nhe\notesoff|^cer es^ta ver|^dade
      ^Para po|^der ser ^verda|^dei^ro
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu su|^bi ser^ra de es|pinhos \altchords{\id[2]{(B)}|B | \e}
      ^Pisan|^{do\jw em} pon^tas a|^gu^das \altchords{|G#m (B) |F#}
      \noteson\[<C#>]As \[<E>]es\notesoff|^trelas ^me dis|^seram \altchords{|B |(F#)}
      ^No mun|^do se ^cura |^tu^do \altchords{B |G#m F# |E}
    \endrep
  \endverse
  \beginverse
    \beginrep
      As es|^trelas ^me dis|seram
      ^Ouve |^muito\jw e ^fala |^pou^co
      \noteson\[<C#>]Pa\[<E>]ra \notesoff|^poder ^compreen|^der
      E ^conver|^sar com ^meus ca|^bo^clos
    \endrep
  \endverse
  \beginverse
    \beginrep
      Os ca|^boclos ^já che|garam
      De ^braços |^nus e ^pés no |^chão ^
      \noteson\[<C#>]E\[<E>]les \notesoff|^trazem re^médios |^bons
      ^Para |^curar ^os cris|^tã^os
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "As Estrelas"
    %   composer = "Mestre Irineu Serra"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative e'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key a \major \time 4/4 \partial 4
      \repeat volta 2 {
        cis8\pomark e | a,4 a cis e | a, a8( a) a'4( a8) gis8
        | fis4. e8 e4. cis8 | e2( e8) \parenthesize cis8 << \parenthesize cis8 \once\slurUp \single\altcol e8 \single\altcol ( >>  e8)
        | a,4 a8( a8) cis4 e | e4.( e8) a4( a8) gis8
        | fis4. e8 e4. cis8 | d2( d4)
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        \set includeGraceNotes = ##t
        As es -- | tre -- las já che -- | ga -- ram; __ _
        Pa -- ra di -- | zer o no -- me | seu; __ _ ""
        \altcol Sou __ _  | eu, sou __ _ eu, sou | eu; __ _
        Sou __ _ eu | um fi -- lho de | Deus. __ _
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      \repeat volta 2 {
        As es -- | tre -- las me le -- | va -- ram; __ _
        Pa -- ra cor -- | rer o mun -- do~in -- | tei -- ro;
        Pra co -- nhe -- | cer es -- _ ta ver -- | da -- de;
        Pa -- ra po -- | der ser ver -- da -- | dei -- ro.
      }
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      \repeat volta 2 {
        Eu su -- | bi ser -- ra de~es -- | pi -- nhos; __ _
        Pi -- _ san -- | do~em pon -- tas a -- | gu -- das; ""
        As es -- | tre -- las __ _ me dis -- | se -- ram;
        No __ _ mun -- | do se cu -- ra | tu -- do.
      }
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      \repeat volta 2 {
        As es -- | tre -- las me dis -- | se -- ram; __ _
        Ou -- _ ve | mui -- to~e fa -- la | pou -- co; ""
        Pa -- ra | po -- der __ _ com -- preen -- | der;
        E con -- _ ver -- | sar com meus ca -- | bo -- clos.
      }
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5."
      \repeat volta 2 {
        Os ca -- | bo -- clos já che -- | ga -- ram; De
        bra -- _ ços | nus e pés no | chão; __ _ ""
        E -- les | tra -- zem re -- mé -- dios | bons; __ _
        Pa -- _ ra | cu -- rar os cris -- | tã -- os.
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4 | a1 | a1 | fis2:m \parenthesize a2 | e1
        | a | \parenthesize e2 a2 | fis2:m e2 | d2.
      }
    }
    \setStaffSize #14 % to fit translation, too
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \iflyriconly\forcebrk\fi
  \begin{translation}[EN]
    The Stars already arrived to say their name
    I am, I am, I am, I am a son of God
    \nextverse
    The Stars took me to go through the entire world
    For me to know this Truth, to be able to be true
    \nextverse
    I climbed a hill of thorns, stepping on sharp points
    The Stars told me: in the world everything can be healed
    \nextverse
    The Stars told me: listen much and speak little
    So that I can understand and speak with my caboclos
    \nextverse
    Thge caboclos already arrived with naked arms and feet on the ground
    They bring good remedies to heal the Christians
  \end{translation}
\endsong


\beginsong{Os Espíritos Estão Chegando}[id={os-espiritos-estao-chegando-95ut1o},by={Alex Polari},key={C},gk={C},tags={mazurca}]
  \audio{https://www.nossairmandade.com/hymn/948/OsEsp\%C3\%ADritosEst\%C3\%A3oChegando}
  \newchords{chords_osespiritosestao_a}\newchords{chords_osespiritosestao_b}
  \meter{6}{4}
  \mnbeginverse
    \beginrep\memorize[chords_osespiritosestao_a]
      \[^<C>]Pe|\[C]rante este mo\[<G>G]mento \[^<A>]eu |\[^<G>]que\[^<E>]ro \[^<C>]de\[^<A>]cla\[<C>C]rar \altchords{\id[1]{(D)}|D A | - D}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_osespiritosestao_b]
      \[^<G>]U|\[<C>C]mas pou\[^<E>]cas \[^<D>]pa\[<* C>]lavras \[^<G>]que |\[<B>G]eu ouvi \[^<A>]pub\[^<B>]li\[<C>C]car \altchords{|D |A D}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Os es|^píritos estão che^gando pela |linha deva^gar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Se pre|^parem apa^relhos para\jw ou|^vir e ensi^nar
    \endrep
  \endverse
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Eles es|^tão vin do do ^Céu, tam|bém virão do ^Mar
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Da |^Terra e das Mon^tanhas para |^todos doutri^nar
    \endrep
  \endverse
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Esta es|^trada é do ^Mestre, foi |ele quem a^briu
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Replan|^tando\jw a Santa Dou^trina para\jw a|^quele que se^guir
    \endrep
  \endverse
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Santo |^Daime, Santa Ma^ria, con|sola e dá a^lívio
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Eu |^digo aos meus ca^boclos que a|^qui o centro\jw é ^livre
    \endrep
  \endverse
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Todos |^são filhos de ^Deus, só |basta ele que^rer
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Sa|^údo a prote^ção do Vo|^vô Omungue^lê
    \endrep
  \endverse
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      No |^meio da minha vi^agem ele co|migo veio ^ter
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Disse |^que já fora es^cravo e cultu|^ava o mesmo po^der
    \endrep
  \endverse
  \iflyriconly\else\hardbrk\fi
  \beginverse\replay[chords_osespiritosestao_a]
    \beginrep
      Meu Im|^pério Jurami^dam, que |glória vos perten^cer
    \endrep
    \endverse\glueverses\beginverse\replay[chords_osespiritosestao_b]
    \beginrep
      Basta |^ser sempre fi^el que |^tudo eu posso ^ser
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Os Espíritos Estão Chegando"
    %   composer = "Alex Polari"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative c''' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 6/4 \partial 4.
      \sectionLabel "A"
      \repeat volta 2 {
        \parenthesize c8\pomark c8( c8) | c8( c8) c8( c8) c c  g4( g8)( g8) a8( a8)
        | g4 e8( e8) c8 a8  c4( c8)
      }
      \sectionLabel "B"
      \repeat volta 2 {
        r8 g8( g8) | c4( c8)( c8) e8 d8  c4( c8)( c8) g8( g8)
        | b8( b8) b8( b8) a8 b8  c4( c8)
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
        \skip 1 Pe -- _ | ran -- _ te es -- te mo -- men -- to __ _
        Eu __ _ | que -- ro __ _ de -- cla -- rar. __ _
      \set stanza = "1.B"
        U -- _ | mas pou -- _ cas pa -- la -- vras __ _
        Que __ _ | eu __ _ ou -- vi pu -- bli -- car. __ _
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
        \skip 1 Os es -- | pí -- ri -- tos es -- tão che -- gan -- do __ _
        Pe -- la | li -- nha __ _ de -- va -- gar. __ _
      \set stanza = "2.B"
        Se pre -- | pa -- rem __ _ a -- pa -- re -- lhos __ _
        Pa -- ra~ou -- | vir __ _ e __ _ en -- si -- nar. __ _
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
        E -- les es -- | tão __ _ vin -- _ do do Céu __ _ _
        Tam -- _ | bém vi _ -- rão do Mar. __ _
      \set stanza = "3.B"
        Da __ _ | Ter -- ra e das Mon -- ta -- nhas __ _
        Pa -- ra | to _ -- dos __ _ dou -- tri -- nar. __ _
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
        Es -- ta es -- | tra -- _ da __ _ é do Mes -- tre __ _
        Foi __ _ | e -- le __ _ quem a -- bri -- u.
      \set stanza = "4.B"
        Re -- plan | tan -- do~a San -- ta Dou -- tri -- na __ _
        Pa -- ra~a -- | que _ -- le __ _ que se -- guir. __ _
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5."
        \skip 1 San -- to | Dai -- _ me, San -- ta Ma -- ri -- a __ _
        Con -- _ | so -- la e dá a -- lí -- vio.
      \set stanza = "5.B"
        Eu __ _ | di -- go aos meus ca -- bo -- clos __ _
        Que a -- | qui __ _ o __ _ cen -- tro~é li -- vre.
    }
    theLyricsSix = \lyricmode {
      \set stanza = "6."
        \skip 1 To -- dos | são __ _ fi -- _ lhos de Deus __ _ _
        Só __ _ | bas -- ta e -- le que -- rer. __ _
      \set stanza = "6.B"
        Sa -- _ | ú -- do a pro -- te -- ção __ _ _
        Do  Vo -- | vô __ _ O -- _ mun -- gue -- lê. __ _
    }
    theLyricsSeven = \lyricmode {
      \set stanza = "7."
        \skip 1 No __ _ | mei -- o da mi -- nha vi -- a -- gem e --
        le co -- | mi -- go __ _ vei -- o ter. __ _
      \set stanza = "7.B"
        Dis -- se | que já fo -- ra es -- cra -- vo
        E cul -- tu -- | a -- va o mes -- mo po -- der. __ _
    }
    theLyricsEight = \lyricmode {
      \set stanza = "8."
        \skip 1 Meu Im -- | pé -- _ rio Ju -- ra -- mi -- dam __ _ _
        Que __ _ | gló -- ria vos per -- ten -- cer. __ _
      \set stanza = "8.B"
        Bas -- ta | ser __ _ sem -- pre fi -- el __ _ _
        Que __ _ | tu _ -- do eu pos -- so ser. __ _
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4. | c2. g
        | g c4.
      }
      \repeat volta 2 {
        s4. | c2. c
        | g c4.
      }
    }
    \setStaffSize #15.5 % for better division
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}[EN]
    In this very moment I want to declare
    A few words which I heard being announced
    \nextverse
    The Spirits are arriving along the line slowly
    Prepare yourselves to listen and teach
    \nextverse
    They are coming from the Sky, they will also come from the Sea
    The Earth and the mountains to indocrinate everyone
    \nextverse
    This road is the Master's, it was he who opened it
    Replanting the Holy Doctrines for those who followed
    \nextverse
    Holy Daime and Holy Mary who console and give relief
    I tell my caboclos that the center here is free
    \nextverse
    All are children of God, it is sufficient that He wills it
    I praise the protection of grandfather Omunguelê
    \nextverse
    In the middle of my journey he come to meet me
    Told me that he had been slave and cultivated the same power
    \nextverse
    My Emperor Juramidam, what a glory to belong to You
    I need only to be forever faithful to be everything I can be
  \end{translation}
\endsong


\beginsong{Linha do Arroxim}[id={linha-do-arroxim-psqgav},by={Vera Fróes},key={C},gk={D, C--D}]
  \audio{https://www.nossairmandade.com/hymn/58/LinhaDeArrochim}
  \audio[key={C}]{https://www.youtube.com/watch?v=6L4aBI5EWzo}
  \newchords{chords_linhadoarroxim_a}\newchords{chords_linhadoarroxim_b}
  \mnbeginverse
    \beginrep\memorize[chords_linhadoarroxim_a]
      \[^<G>]Harmo|\[<C>C]nia, Ver\[<*>]dade\jw e \[^<E>]Per|\[<G>(G)]dão \[<*>]{} \altchords{\id[1]{(D)}|D |(A)}
      São \[^<A>]os \[^<G>]três |\[<E>C]pon\[^<C>]tos que se\[<A>Am]gu\[^<B>]ram\jw es\[^<C>]ta\jw u\[^<A>]ni|\[<G>G]ão \[<*>]{} \altchords{|D Bm |A}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_linhadoarroxim_b]
      \[^<C>]Eu |\[<A>Am]peço \[^<C>]meus ir\[C]mãos: prestem aten|\[<G>G]ção \altchords{|Bm D |A}
      Que o \[<C>C]Mestre do \[^<D>]as|\[<E>(Em)]tral está \[^<C>]o\[<D>G]lhando a \[^<B>]ses|\[<C>C]são \[<*>]{} \altchords{D |(F#m) A |D}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_linhadoarroxim_a]
    \beginrep
      As do|^enças ^que apare|^cer ^
      É disci|^plina pra quem ^faz por mere|^cer ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadoarroxim_b]
    \beginrep
      Pe|^dir aos es^píritos cura|^dores
      da ^linha de\jw Arro|^xim a Jesus ^Cristo Reden|^tor ^
    \endrep
  \endverse
  \beginverse\replay[chords_linhadoarroxim_a]
    \beginrep
      Ter |^fé\jw e espe^rança no pe|^di^do
      Pensar em |^Deus e na ^nossa Virgem |^Mãe ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadoarroxim_b]
    \beginrep
      Não |^ter ^medo de mor|^rer
      ^E se sair cor|^rendo é pi^or para vo|^cê ^
    \endrep
  \endverse
  \beginverse\replay[chords_linhadoarroxim_a]
    \beginrep
      Com |^calma ^e tranqüili|^da^de
      O seu ca|^minho ^vai se\jw ilumi|^nar ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadoarroxim_b]
    \beginrep
      Te |^lembra do ve^lho Jurami|^dam
      Ele\jw es^tá sempre pre|^sente e se^gura\jw a tua |^mão
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Linha do Arroxim"
    %   composer = "Vera Fróes"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative g' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 4
      \sectionLabel "A"
      \repeat volta 2 {
        g8(\pomark g8) | c4( c8) c c( c) c e | g2( g8) g a g
        | \once\slurUp e( << \once\slurUp e8)( \single\altcol c \altmark "1-2." >> << e \single\altcol c \altmark "1-3.") >> c a b c a | g2.
      } \break
      \sectionLabel "B"
      \repeat volta 2 {
        c4 | a8( a) \parenthesize c8 \parenthesize c c16( c16) c8 c8 c | g8( g8) \parenthesize g8 g8 c c c d
        | e e e c d d d b | c2.
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        Har -- mo -- | ni -- a, Ver -- da _ -- de~e Per -- | dão; __ _
        São os três | pon -- \altcol tos \altcol que se -- gu -- ram~es -- ta~u -- ni -- | ão.
      }
      \set stanza = "1.B"
      \repeat volta 2 {
        Eu | pe -- ço meus ir -- mãos: __ _
        prestem a -- ten -- | ção; __ _
        Que o Mes -- tre do as -- | tral
        es -- tá o -- lhan -- do a ses -- | são.
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      \repeat volta 2 {
        As do -- | en -- _ ças que a -- pa -- re -- | cer; __ _
        É dis -- ci -- | pli -- \altcol na \altcol Pra quem faz por me -- re -- | cer.
      }
      \set stanza = "2.B"
      \repeat volta 2 {
        Pe -- | dir __ _ aos es -- pí -- ri -- tos cu -- ra -- | do _ -- res;
        Da li -- nha de~Ar -- ro -- | xim
        A Je -- sus Cris -- to Re -- den -- | tor.
      }
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      \repeat volta 2 {
        Ter __ _ | fé~e es -- pe -- ran -- ça no pe -- | di -- do;
        Pen -- sar em | Deus __ _ \altcol e na nos -- sa Vir -- gem | Mãe.
      }
      \set stanza = "3.B"
      \repeat volta 2 {
        Não | ter __ _ "" "" me _ -- do de mor -- | rer; __ _
        "" E se sa -- ir cor -- | ren -- do é pi -- or pa -- ra vo -- | cê.
      }
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      \repeat volta 2 {
        Com __ _ | cal -- _ ma e tran -- qüi -- li -- | da -- de;
        O seu ca -- | mi -- _ _ nho vai se~i -- lu -- mi -- | nar.
      }
      \set stanza = "4.B"
      \repeat volta 2 {
        Te | lem -- bra do ve -- lho __ _ Ju -- ra -- mi -- | dam; __ _
        E -- le~es -- tá sem -- pre pre -- | sen -- te e se -- gu -- ra~a tu -- a | mão.
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4 | c1 | \parenthesize g1 | c2 a2:m | g2.
      }
      \repeat volta 2 {
        s4 | a2:m c2 | g2 c2 | \parenthesize e2:m g2 | c2.
      }
    }
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}[EN]
    Harmony, truth and forgiveness
    Are the three points that secure this union.
    I ask my brothers and sisters: pay attention
    For the Master in the Astral is watching this session
    \nextverse
    The sicknesses that appear
    The discipline for those who try to be deserving.
    It's to ask the spiris of healing
    from the Arroxim line and Jesus Christ the Redeemer
    \nextverse
    To have faith and hope in the request
    To think of God and of our Virgin Mother.
    To not be afraid of dying
    And if you take off running, it's worse for you.
    \nextverse
    With calmness and tranquility
    Your path is going to be illuminated.
    Remember the old Juramidam
    He is always present and holds your hand.
  \end{translation}
\endsong


\beginsong{Linha do Tucum}[id={linha-do-tucum-k4s4ch},by={Mestre Irineu Serra},tags={marcha e meia valsa, 2x},key={Dm},gk={Dm, Cm--Gm}]
  \audio{https://www.nossairmandade.com/hymn/59/LinhaDoTucum}
  \newchords{chords_linhadotucum_a}\newchords{chords_linhadotucum_b}
  \mnbeginverse
    \beginrep\memorize[chords_linhadotucum_a]
      \[^<A>]Eu |\[<D>Dm]canto \[^<C#>]a\[<* D>]qui \[^<E>]na |\[^<F>]ter\[^<D>]ra \[<*>]{} \altchords{\id[1]{(Em)}|Em | \e}
      O \[^<F>]a|\[<E>A7]mor \[^<D>]que \[<* C#>]Deus \[^<E>]me |\[<D>Dm]dá \[<*>]{} \altchords{|B7 |Em}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_linhadotucum_b]
      \[^<A>]Pa\[^<C>]ra |\[C]sempre\[<*>]{} \[^<B&>]pa\[^<A>]ra |\[<B&>F]sem\[^<A>]pre \[<*>]\altchords{|D |G}
      Para |\[A7]sempre\[^<*>]{} para |\[<D>Dm]sempre \[<*>]\altchords{|B7 |Em}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_linhadotucum_a]
    \beginrep
      A minha |^Mãe que ^vem co|migo ^
      Que me |^deu es^ta li|^ção ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadotucum_b]
    \beginrep
      Para |^sempre^ para |^sempre^
      Para |^sempre eu \noteson\[<* C#>]ser \[<E>]ir\notesoff|^mão ^
    \endrep
  \endverse
  \beginverse\replay[chords_linhadotucum_a]
    \beginrep
      Enxo|^tando\jw os ^malfa|zejos ^
      Que não |^querem ^me ou|^vir ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadotucum_b]
    \beginrep
      Que escu|^recem^ o pensa|^mento^
      E nunca |^podem \noteson\[<* C#>]ser \[<E>]fe\notesoff|^liz ^
    \endrep
  \endverse
  \beginverse\replay[chords_linhadotucum_a]
    \beginrep
      Esta\jw é a |^linha ^do Tu|cum ^
      Que traz |^toda ^leal|^da^de
    \endrep
    \endverse\glueverses\beginverse\replay[chords_linhadotucum_b]
    \beginrep
      Casti|^gando^ os menti|^rosos^
      Aqui den|^tro des\noteson\[<* C#>]ta \[<E>]ver\notesoff|^da^de
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Linha do Tucum"
    %   composer = "Mestre Irineu Serra"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative b' {
      \set melismaBusyProperties = #'() \slurDashed
      \key d \minor \time 4/4 \partial 4.
      \sectionLabel "1.A"
      \repeat volta 2 {
        r8 a4 | d4 d8 cis8 d4 e | f d2 d8 f8
        | e4 d4 cis4 e4 | d2~ d8
      } \break
      \sectionLabel "1.B"
      \repeat volta 2 {
        r8 a8 c8 | c4 c2 bes8 a8 | bes4 a2 a8 a8 | a4 a2 a8 a8 | d4 d4.
      } \break
      \sectionLabel "2-4.A"
      % Can't use "\repeat segno" construct here, as it conflicts with
      % the section label, thus using \segno and \dalsegno macros.
      \repeat volta 3 {
        \repeat volta 2 {
          \parenthesize a8\pomark\segno a8 a8 | d4 cis4 d4 e | f( d2) d8 f8
          | e4 d4 cis4 e4 | d2( d8)
        } \break
        \sectionLabel "2-4.B"
        \repeat volta 2 {
          \parenthesize a8 a8 c8 | c4 c4. \parenthesize bes8 bes8 a8 | bes4 a4. \parenthesize a8 a8 a8 | a4 a8( a8) cis4 e4 | d2( d8)
          \dalsegno
        }
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1.A"
        Eu | can -- to a -- qui na | ter -- ra;
        O a -- | mor que Deus me | dá. _
      \set stanza = "1.B"
        Pa -- ra | sem -- pre pa -- ra | sem -- pre;
        Pa -- ra | sem -- pre pa -- ra | sem -- pre.
      <<
        \new Lyrics {
          \set associatedVoice = "theMelody"
          \set stanza = "2.A"
            A mi -- nha | Mãe que vem co -- | mi -- go;
            Que me | deu es -- ta li -- | ção. _
          \set stanza = "2.B"
            "" Pa -- ra | sem -- pre "" pa -- ra | sem -- pre; ""
            Pa -- ra | sem -- pre eu ser ir -- | mão. _
        }
        \new Lyrics {
          \set stanza = "3.A"
            "" En -- xo -- | tan -- do~os mal -- fa -- | ze -- jos;
            Que não | que -- rem me ou -- | vir. _
          \set stanza = "3.B"
            Que es -- cu -- | re -- cem o pen -- sa -- | men -- to;
            E nun -- ca | po -- dem __ _ ser fe -- | liz. _
        }
        \new Lyrics {
          \set stanza = "4.A"
            Es -- ta~é a | li -- nha do Tu -- | cum; __ _
            Que traz | to -- da le -- al -- | da -- de.
          \set stanza = "4.B"
            "" Cas -- ti -- | gan -- do os men -- ti -- | ro -- sos;
            A -- qui den -- | tro des -- _ ta ver -- | da -- de.
        }
      >>
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4. | d1:m | d:m | a:7 | d2:m~ d8:m
      }
      \repeat volta 2 {
        s4. | c1 | f1 | a:7 | d2:m~ d8:m
      }
      \repeat volta 2 {
        s4. | d1:m | d:m | a:7 | d2:m~ d8:m
      }
      \repeat volta 2 {
        s4. | c1 | f1 | a:7 | d2:m~ d8:m
      }
    }
    \setStaffSize #14.5 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]
    I sing here on earth, the love that God gives me
    Forever, forever, forever, forever
    \nextverse
    My Mother that comes with me, who gave me this lesson
    Forever, forever, forever to be a brother
    \nextverse
    Driving away those who do evil, who don't want to listen to me
    Who darken their minds and can never be happy
    \nextverse
    This is the thread of Tucum that brings all loyalty
    Punishing the liars here in this truth
  \end{translation}
\endsong


\beginsong{Olho Para o Firmamento}[id={olho-para-o-firmamento-f84e5q},by={Alfredo},key={Am},gk={Am, Gm--Am}]
  \audio[key=Am]{https://www.nossairmandade.com/hymn/475/OlhoParaOFirmamento}
  \newchords{chords_olhopara_a}\newchords{chords_olhopara_b}
  \mnbeginverse
    \beginrep\memorize[chords_olhopara_a]
      \[^<E>]Olho |\[<A>Am]para\jw o \[<*>]fir\[^<B>]ma|\[^<C>]men\[^<A>]to\jw \altchords{\id[1]{(Gm)}|Gm | \e}
      E \[<* E>]ve\[^<G>]jo |\[G]to\[^<D>]do \[(E7/D)]U\[^<A>]ni|\[Am]ver\[<*>]so \altchords{|F (D7/C) |Gm}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_olhopara_b]
      \[^<A>]Es\[^<B>]ta |\[<C>C]força \[<B>E]que re|\[<A>Am]cebo \altchords{|B& D |Gm}
      \[<* C>]Sem\[^<A>]pre |\[<E>E]canto\jw e \[<* B>]sem\[^<C>]pre |\[<A>Am]pe\[<*>]ço \altchords{|D |Gm}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_olhopara_a]
    \beginrep
      Peço |^ao O^nipo|tente
      ^E a |^Ele ^{me hu}|^mi^lho
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olhopara_b]
    \beginrep
      Para |^nunca ^me fal|^tar
      ^Vossa |^luz com\jw o ^santo |^bri^lho
    \endrep
  \endverse
  \beginverse\replay[chords_olhopara_a]
    \beginrep
      Me se|^guro ^no As|tral \altchords{\id[2]{(Em)}|Em | \e}
      E ^quando |^me a^{cho do}|^en^te \altchords{|D (B7/A) |Em}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olhopara_b]
    \beginrep
      Me lem|^bro de ^minha |^Mãe \altchords{|G B |Em}
      E ^de meu |^Pai O^nipo|^ten^te \altchords{|B |Em}
    \endrep
  \endverse
  \beginverse\replay[chords_olhopara_a]
    \beginrep
      Estas |^forças ^verda|deiras
      ^Provam |^que são ^exis|^ten^tes
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olhopara_b]
    \beginrep
      Mas quem |^vive ^neste |^mundo
      ^Pouco |^vê e ^pouco |^sen^te
    \endrep
  \endverse
  \brk
  \beginverse\replay[chords_olhopara_a]
    \beginrep
      Digo |^tudo ^humi|lhado\jw \altchords{\id[3]{MK (Am)}|Am |C}
      E ^conti|^nuo ^{a jor}|^na^da \altchords{|G Dm |Am}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olhopara_b]
    \beginrep
      Com meu |^Pai em ^minha |^frente \altchords{|Am E7 |Am}
      ^E mi|^nha Mãe ^a meu |^la^do \altchords{|- E7 |Am}
    \endrep
  \endverse
  \beginverse\replay[chords_olhopara_a]
    \beginrep
      Vou se|^guindo\jw e ^vou pe|dindo
      ^Para |^todos ^ino|^cen^tes
    \endrep
    \endverse\glueverses\beginverse\replay[chords_olhopara_b]
    \beginrep
      Peço |^força ^nos tra|^balhos
      ^Aos três |^Reis do ^Ori|^en^te
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Olho Para o Firmamento"
    %   composer = "Alfredo"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative e'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key a \minor \time 4/4 \partial 4
      \sectionLabel "A"
      \repeat volta 2 {
        e8\pomark e | a4 a a b | c a e g | g d d a' | a2 a4
      }
      \sectionLabel "B"
      \repeat volta 2 {
        a,8 b | c4 c b b | a4. \parenthesize a8 c4 a | e'4 e4 b4 c | a2 a4
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1.A"
      \repeat volta 2 {
        O -- lho | pa -- ra~o fir -- ma -- | men -- to;~E
        ve -- jo | to -- do U -- ni -- | ver -- so.
      }
      \set stanza = "1.B"
      \repeat volta 2 {
        Es -- ta | for -- ça que re -- | ce -- bo;
        Sem -- pre | can -- to~e sem -- pre | pe -- ço.
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2.A"
      \repeat volta 2 {
        Pe -- ço | ao O -- ni -- po -- | ten -- te;
        E a | E -- le me hu -- | mi -- lho.
      }
      \set stanza = "2.B"
      \repeat volta 2 {
        Pa -- ra | nun -- ca me fal -- | tar; _
        Vos -- sa | luz com~o san -- to | bri -- lho.
      }
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3.A"
      \repeat volta 2 {
        Me se -- | gu -- ro no As -- | tral;
        E quan -- do | me a -- cho do -- | en -- te.
      }
      \set stanza = "3.B"
      \repeat volta 2 {
        Me lem -- | bro de mi -- nha | Mãe;
        E de meu | Pai O -- ni -- po -- | ten -- te.
      }
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4.A"
      \repeat volta 2 {
        Es -- tas | for -- ças ver -- da -- | dei -- ras;
        Pro -- vam | que são e -- xis -- | ten -- tes.
      }
      \set stanza = "4.B"
      \repeat volta 2 {
        Mas quem | vi -- ve nes -- te | mun -- do;
        Pou -- co | vê e pou -- co | sen -- te.
      }
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5.A"
      \repeat volta 2 {
        Di -- go | tu -- do hu -- mi -- | lha -- do;~E
        con -- ti -- | nu -- o a jor -- | na -- da.
      }
      \set stanza = "5.B"
      \repeat volta 2 {
        Com meu | Pai em mi -- nha | fren -- te;
        E mi -- | nha Mãe a meu | la -- do.
      }
    }
    theLyricsSix = \lyricmode {
      \set stanza = "6.A"
      \repeat volta 2 {
        Vou se -- | guin -- do~e vou pe -- | din -- do;
        Pa -- ra | to -- dos i -- no -- | cen -- tes.
      }
      \set stanza = "6.B"
      \repeat volta 2 {
        Pe -- ço | for -- ça nos tra -- | ba -- lhos;
        Aos três | Reis do O -- ri -- | en -- te.
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4 | a1:m | a1:m | g2 \parenthesize e2:7/d | a2.:m
      }
      \repeat volta 2 {
        s4 | c2 e2 | a1:m | e | a2.:m
      }
    }
    \setStaffSize #15 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}[EN]
    I look at the firmament, I see the whole Universe
    This force that I receive, I always sing and always ask
    \nextverse
    I ask the Omnipotent, and to Him I humble myself
    That I never lack your light with yoour holy brilliance
    \nextverse
    I secure myself in the Astral, and when I find myself sick
    I remember my Mother and my Omnipotent Father
    \nextverse
    These true forces prove that they exist
    But who lives in this world sees little and feels little
    \nextverse
    I say everything with humility and continue the joourney
    With my Father in front and my Mother at my side
    \nextverse
    I keep following and asking for all the innocents
    I ask for strength in the works of the three Kings of the Orient
  \end{translation}
\endsong


\beginsong{Marachimbé \\ Sol, Lua, Estrela}[id={marachimbe-hntl0i},by={Alfredo},key={D},gk={D, C--G}]
  \audio{https://www.nossairmandade.com/hymn/512/Sol,Lua,Estrela}
  \mnbeginverse
    \beginrep\memorize
      |\[<D>D]Sol, \[<*>]Lua, Es|tre\[<A>A]la \altchords{\id[1]{(E)}|E | - B}
      \[^<C#>]A |\[<E>Em]Terra, o \[<*>]Ven\[^<F#>]to\jw e o |\[^<E>]Mar \[A7]{} \altchords{|F#m | - B7}
      \[^<A>]A Flo|\[<D>D]resta e \[<*>]seus mis|térios \altchords{|E | \e}
      \[<* A>]Para |\[<F E>Fmaj7]todos \[<C# E>A7]estu|\[<D>D]dar \[<*>]{} \altchords{|Gmaj7 B7 |E}
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    \beginrep
      |^Sol, ^Lua, Es|tre^la \altchords{\id[2]{(F)}|F | - C}
      A |^Terra, o ^Vento\jw e o |Mar ^ \altchords{|Gm | - C7}
      É a |^Luz do ^firma|mento\jw \altchords{|F | \e}
      É ^só a |^quem eu ^devo a|^mar ^ \altchords{|A&maj7 C7 |F}
    \endrep
  \endverse
  \beginverse
    \beginrep
      É só a |^quem eu ^devo a|mar
      E ^me se|^gura ^em meu |can^to
      Pai, |^Filho e ^Virgem |Mãe
      Com ^o di|^{vino Es}^pírito |^San^to
    \endrep
  \endverse
  \beginverse
    \beginrep
      O di|^vino Es^pírito |Santo\jw
      Em ^todos |^três num ^só se en|cer^ra
      É o |^tempo ^do a|puro
      ^Pouca |^{paz e} ^muita |^guer^ra
    \endrep
  \endverse
  \beginverse
    \beginrep
      Esta |^paz é ^da Ra|inha
      ^Da Flo|^resta ^e do |Mes^tre
      É o |^princí^pio das |dores
      ^Meus ir|^mãos, o ^tempo é |^es^te
    \endrep
  \endverse
  \beginverse
    \beginrep
      Meus ir|^mãos, o ^tempo é |este \altchords{\id[3]{O (D)}|D | \e}
      ^Das lem|^branças ^de No|é ^ \altchords{|Em |A7 \e}
      Peço |^força a ^todos |seres \altchords{|D | \e}
      ^Firmo |^{em Ma}^rachim|^bé ^ \altchords{|Em A7 |D}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[]
    %% transcribed by larva, latest update on 2025-06
    % \header {
    %   title = "Marachimbé"
    %   composer = "Alfredo"
    % }
    \include "ulsbs-include-head.ly"
    theMelody = \relative d'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key d \major \time 4/4 \partial 4.
      \repeat volta 2 {
        \parenthesize a8 \parenthesize a8\pomark \parenthesize a8 | d4( d8)( d8) d8( d8) d8( d8) | d4.( d8) a4 cis | e4 e8( e8) e4 fis8( fis8) | e2( e4) a,8( a8)
        | d4 d8( d8) d4 d4 | d4. d8 a4 a4 | f'4 e8( e8) cis8( cis8) e8( e8) | d2( d8)
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \skip 1 \skip 1 \skip 1 | Sol, __ _ _ Lu _ -- a, Es -- | tre -- _ la;
      A | Ter -- ra, o Ven -- to~e o | Mar; __ _
      A Flo -- | res -- ta e seus mis -- | té -- rios;
      Pa -- ra | to -- dos __ _ es _ -- tu _ -- | dar. __ _
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      \skip 1 \skip 1 \skip 1 | Sol, __ _ _ Lu _ -- a, Es -- | tre -- _ la;
      A | Ter -- ra, o Ven -- to~e o | Mar; __ _
      É a | Luz do __ _ fir -- ma -- | men -- to;~É
      só a | quem eu __ _ de _ -- vo a -- | mar. __ _
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      É só a | quem eu __ _ de _ -- vo a -- | mar;
      E me se -- | gu -- ra __ _ em meu __ _ | can -- to;
      Pai, __ _ | Fi -- lho e Vir -- gem | Mãe;
      Com o di -- | vi -- no Es -- pí -- ri -- to __ _ | San -- to.
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      \skip 1 O di -- | vi -- no Es -- pí -- ri -- to __ _ | San -- to;~Em
      to -- dos | três num __ _ só se en -- | cer -- ra;
      É o | tem -- po __ _ do a -- | pu -- ro;
      Pou -- ca | paz e __ _ mui _ -- ta __ _ | guer -- ra.
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5."
      \skip 1 Es -- ta | paz é __ _ da __ _ Ra -- _ | i -- nha;
      Da Flo -- | res -- ta __ _ e do __ _ | Mes -- tre;
      É o | prin -- cí -- _ pio das | do -- res;
      Meus ir -- | mãos, o __ _ tem _ -- po é | es -- te.
    }
    theLyricsSix = \lyricmode {
      \set stanza = "6."
      \skip 1 Meus ir -- | mãos, o __ _ tem _ -- po é | es -- te;
      Das lem -- | bran -- ças __ _ de No _ -- | é; __ _
      Pe -- ço | for -- ça a to -- dos | se -- res;
      Fir -- mo em Ma _ -- ra _ -- chim _ -- | bé. __ _
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4. | d1 | d2 a2 | e1:m | e2:m a2:7
        | d1 | d | f2:maj7 a:7 | d2~ d8
      }
    }
    \setStaffSize #14 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \ifshowlilypond\translationoff\fi % to fit whole song on one spread
  \begin{translation}[EN]
    Sun, Moon, Star; the Earth, the wind and the sea
    The forest and its mysteries for everyone to study
    \nextverse
    Sun, Moon, Star; the Earth, the wind and the sea
    It is the light of the firmament, it is only this that I must love
    \nextverse
    It is only this that I must love, and secures me in my song
    Father, Son and Virgin Mother, with the Divine Holy Spirit
    \nextverse
    The Divine Holy Spirit, all three enclosed in one
    It is the time of purification, little peace and much war
    \nextverse
    The peace is of the Queen of the Forest and of the Master
    It is the beginning of the suffering; my brothers, this is the time
    \nextverse
    My brothers, this is the time to remember the times of Noah
    I ask strength of all beings, I am firm in Marachimbé
  \end{translation}
\endsong


\beginsong{Eu Agora Paro Peço}[id={eu-agora-paro-peco-uu5kes},by={Alfredo},key={D},gk={D, D--E}]
  \audio{https://www.nossairmandade.com/hymn/541/EuAgoraParoEPe\%C3\%A7o}
  \newchords{chords_euagoreparopeco_a}\newchords{chords_euagoreparopeco_b}
  \mnbeginverse
    \beginrep\memorize[chords_euagoreparopeco_a]
      \[^<A>]Eu a|\[<D>D]gora \[<*>]pa\[^<F#>]{ra o} |\[^<A>]pe\[^<F#>]ço
      \[<*>]Pe\[^<D>]{ço e} |rogo ao \[<B>Bm]Se\[^<D>]nhor |\[<A>A]Deus \[<*>]
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_euagoreparopeco_b]
      \[^<A>]Que \[^<F#>]me \[^<G>]dê |\[<A>A]a \[^<B>]san\[<F#>(F#m)]ta \[^<A>]sa|\[A]úde
      \[<*>]Pa\[^<C#>]ra |\[<E>(Em)]todos \[<B>A7]fi\[^<D>]lhos |\[D]Seus \[<*>]
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_euagoreparopeco_a]
    \beginrep
      Aqui |^esta^mos pe|dindo
      E ^quem res|ponde é ^nosso |^Pai ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euagoreparopeco_b]
    \beginrep
      Vossa |^vonta^{de é} |^feita
      ^Sempre |^{em to}^do lu|^gar ^
    \endrep
  \endverse
  \beginverse\replay[chords_euagoreparopeco_a]
    \beginrep
      Sempre |^estou ^reu|nindo
      ^Para |mais fá^{cil di}|^zer ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euagoreparopeco_b]
    \beginrep
      No tra|^balho ^{com fir}|^meza
      ^É mais |^fácil ^de ven|^cer ^
    \endrep
  \endverse
  \beginverse\replay[chords_euagoreparopeco_a]
    \beginrep
      Vou cha|^mando ^um a |um
      ^Aqui |dentro ^{da ses}|^são ^
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euagoreparopeco_b]
    \beginrep
      O que |^tiver ^consci|^ência
      ^E de|^seja ^ser ir|^mão ^
    \endrep
  \endverse
  \brk
  \beginverse\replay[chords_euagoreparopeco_a]
    \beginrep
      Esta |^prova é ^para |todos
      ^Nos e|xige es^{te tra}|^ba^lho
    \endrep
    \endverse\glueverses\beginverse\replay[chords_euagoreparopeco_b]
    \beginrep
      Viver |^sempre com ^ale|^gria
      E ^no sa|^{lão can}^tar hi|^ná^rio
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
    theMelody = \relative d'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key d \major \time 4/4 \partial 4.
      \sectionLabel "A"
      \repeat volta 2 {
        r8 a8 \pomark a8 | d4 d4 d4 fis4 | a4. fis8 fis4 d4
        | d4 d8( d8) b4 d4 | a2( a8)
      }
      \sectionLabel "B"
      \repeat volta 2 {
        \parenthesize a8 fis8 g8 | a4 b8( b8) fis4 a4 | a4. a8 a4 cis4
        | e4 e4 b4 d4 | d2( d8)
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1.A"
      Eu a -- | go -- ra pa -- ra~o | pe -- ço;
      Pe -- ço~e | ro -- go ao Se -- nhor | Deus. __ _
      \set stanza = "1.B"
      Que me dê | a san -- _ ta sa -- | ú -- de;
      Pa -- ra | to -- dos fi -- lhos | Seus. __ _
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2.A"
      A -- qui | es -- ta -- mos pe -- | din -- do;~E
      quem res -- | pon -- de é nos -- so | Pai. __ _
      \set stanza = "2.B"
      \skip 1 Vos -- sa | von -- ta -- _ de é | fei -- ta;
      Sem -- pre | em to -- do lu -- | gar. __ _

    }
    theLyricsThree = \lyricmode {
      \set stanza = "3.A"
      Sem -- pre | es -- tou re -- u -- | nin -- do;
      Pa -- ra | mais fá -- _ cil di -- | zer. __ _
      \set stanza = "3.B"
      \skip 1 No tra -- | ba -- lho __ _ com fir -- | me -- za;
      É mais | fá -- cil de ven -- | cer. __ _

    }
    theLyricsFour = \lyricmode {
      \set stanza = "4.A"
      Vou cha -- | man -- do um a | um; __ _
      A -- qui | den -- tro __ _ da ses -- | são. __ _
      \set stanza = "4.B"
      \skip 1 O que | ti -- ver __ _ cons -- ci -- | ên -- cia;
      E de -- | se -- ja ser ir -- | mão. __ _
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5.A"
      Es -- ta | pro -- va~é pa -- ra | to -- dos;
      Nos e -- | xi -- ge es -- te tra -- | ba -- lho.
      \set stanza = "5.B"
      \skip 1 Vi -- ver | sem -- pre com a -- le -- | gria;
      E no sa -- | lão can -- tar hi -- | ná -- rio.
    }

    theChords = \chordmode {
      \repeat volta 2 {
        s4. | d1 | d1 | d2 b2:m | a2:~ a8:
      }
      \repeat volta 2 {
        s4. | a2 \parenthesize fis2:m | a1 | \parenthesize e2:m a2:7 | d2~ d8
      }
    }
    \setStaffSize #15 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]\iflyriconly\scriptsize\fi% to fit on one page in lyrics-only books
    Now I stop and ask, I ask and pray to the Lord God
    To give me the holy health, for all His sons and daughters
    \nextverse
    Here we are asking, and who answers is our Father
    Your will is done always everywhere
    \nextverse
    I am always reuniting, because it is easier to say
    In work with firmness it is easier to succeed
    \nextverse
    I keep calling one by one here within the session
    Whoever has consciousness and desires to be brother
    \nextverse
    This trial for everyone demands from us this work
    Always to live with joy and to sing the Hinario in the temple
  \end{translation}
\endsong


\beginsong{Eu Te Dei uma Casa}[id={eu-te-dei-uma-casa-u8r2mk},by={Sebastião},key={C},gk={C, B&--C}]
  % Also in songs_astral_concentracao_diversos.tex
  \audio{https://www.nossairmandade.com/hymn/63/EuTeDeiUmaCasa}
  \mnbeginverse
    \beginrep
      \[^<C>]Eu |\[<F>F]peço \[<* A>]a \[^<C>]meu |\[^<A>]Pai\[<*>], \[^<C>]sei que |\[<E>C]Vós \[^<C>]tem \[<* E>]o \[^<G>]po|\[<E>(Em)]der \[<*>]
      \[^<C>]Per\[^<B>]do|\[<A>Am]ai \[^<C>]as \[<E>(A7)]min\[^<G>]has |\[<D>Dm]cul\[<*>]pas, \[^<C>]se \[^<B>]de |\[<A>Am]Vós \[^<C>]eu \[<*>]me\[^<E>]re|\[<C>C]cer\[<*>]
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    \beginrep
      Eu |^te dei ^uma |ca^sa que |^não fal^ta nin|^guém ^\altchords{\id[1]{(B&)}|E& | - |B& |(Dm) }
      Pa|^{ra tu} ^esco|^lher ^aque|^{les que} ^te con|^vém^\altchords{|Gm (G7) |Cm |Gm |B&}
    \endrep
  \endverse
  \beginverse
    \beginrep
      A\jw es|^trada\jw eu ^dou a |to^dos, para |^todos ^via|^jar ^
      |^Ver e ^compreen|^der, ^e fi|^car em ^seu lu|^gar^
    \endrep
  \endverse
  \beginverse
    \beginrep
      |^Eu es^tou com |Deus^, |^Deus es^tá em |^mim ^
      Eu |^estan^{do com} |^Deus^, |^Deus é\jw o ^meu ca|^mi^nho
    \endrep
  \endverse
  \beginverse
    \beginrep
      |^Deus ^é a |gl^ória, |^Deus a ^glória |^é ^
      Quem |^{é meu} ^Salva|^dor ^é Je|^sus de ^Naza|^ré ^
    \endrep
  \endverse
  \beginverse
    \beginrep
      Eu |^vivo ^neste |mun^do na |^minha ^dire|^ção
      \noteson\[<* E>]Ol\[<C>]han\notesoff|\[Am]{do\jw as} coi\[(A7)]{sas de} |\[Dm]Deus e \[<*>]prestan|\[Am]{do bem} \[<*>]aten|\[C]ção\[<*>]
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
    % transcribed by larva, latest update on 2026-04
    theMelody = \relative c'' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 4
      \repeat volta 2 {
         \parenthesize c4\pomark | f( f) a c | a2( a4) \parenthesize c,8( \parenthesize c8) | e4 c e g | e2( e4) \once\slurUp \parenthesize c8 ( << \parenthesize c8) \parenthesize \single\altcol b8 >>
         | a4 c e g | d4.( d8)( d4) \once\slurUp \parenthesize c8( << c8 ) \parenthesize \single\altcol b8 >> | a4 c c e | c2( c4)
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      Eu  |pe -- ço a meu | Pai, __ _
      sei que | Vós tem o po -- | der; __ _
      Per -- \altcol do -- | ai as mi -- nhas | cul _ -- pas,
      se \altcol de | Vós eu me -- re -- | cer. __ _
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      Eu | te dei u -- ma | ca -- sa
      que __ _ | não fal -- ta nin -- | guém; __ _
      Pa -- _ | ra tu es -- co -- | lher __ _
      a -- que -- _ | les que te con -- | vém. __ _
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3."
      A~es -- | tra -- da~eu dou a | to -- dos
      pa -- ra | to -- dos vi -- a -- jar; __ _ "" ""
      Ver e com -- preen -- der __ _
      e fi -- _ car em seu lu -- gar. __ _
    }
    theLyricsFour = \lyricmode {
      \set stanza = "4."
      "" Eu es -- tou com Deus, __ _
      "" "" 1 Deus es -- tá em mim; __ _
      Eu __ _ es -- tan -- do com Deus, __ _ _
      "" "" Deus é~o meu ca -- mi -- nho.
    }
    theLyricsFive = \lyricmode {
      \set stanza = "5."
      "" | Deus __ _ é a | gló _ -- ria, __ _
      | Deus a gló -- ria | é; __ _
      Quem __ _ | é meu Sal -- va -- | dor __ _
      é Je -- _ | sus de Na -- za -- | ré. __ _
    }
    theLyricsSix = \lyricmode {
      \set stanza = "6."
      Eu | vi -- vo nes -- te | mun -- do;
      na __ _ | mi -- nha di -- re -- | ção;
      O -- lhan -- _ | do~as coi -- sas de | Deus e
      pres -- tan -- _ | do bem a -- ten -- | ção. __ _
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s4 | f1 | f1 | c1 | \parenthesize e:m
        | a2:m \parenthesize a:7 | d1:m | a:m | c2.
      }
    }
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]
    I ask my Father, I know that You have the power
    Forgive my guilt, if I am deserving
    \nextverse
    I gave you a house in which no one is missing
    For you to choose those whom you find worthy
    \nextverse
    I give the path to all, for everyone to travel
    To see and understand, and to stay in your place
    \nextverse
    I am with God, God is within me
    Being with God, God is my path
    \nextverse
    God is the glory, God the glory is
    He who is my Savior is Jesus of Nazareth
    \nextverse
    I live in this world following my direction
    Looking at the things of God and paying close attention
  \end{translation}
\endsong


\beginsong{São João na Terra}[id={sao-joao-na-terra-gv9opq},by={Sebastião},tags={2x},key={C},gk={C, C--D}]
  % Also in songs_astral_concentracao_diversos.tex
  \audio[key=D]{https://www.nossairmandade.com/hymn/64/S\%C3\%A3oJo\%C3\%A3oNaTerra}
  \mnbeginverse
    \beginrep
      \[^<G>]Quando\jw ou\[<* C>]vir \[^<E>]fa|\[<G>C]lar de São \[^<E>]Jo\[<* A>]ão \[^<G>]na |\[<E C>Am]terra \altchords{\id[1]{(D)}|D |Bm}
      É \[^<A>]si\[<C>C]nal \[^<E>]de |\[<D>Dm]guerra em \[^<C>]to\[<A>F]do \[^<C>]lu|\[C]gar \altchords{D |Em G |D}
    \endrep
    \beginrep
      \[<*>]{} \ilsl{\[^<C>]É} |\[<D>Dm]fora \[^<C>]de \[<* A>]con\[^<C>]fu|\[<E>C]são \altchords{|Em |D}
      \[^<A>]Che\[<* C>]gou \[^<E>]São |\[<D>Dm]Pedro com \[^<C>]seus \[<A>F]dois \[^<C>]ir|\[C]mãos \altchords{|Em G |D}
    \endrep
  \mnendverse
  \notesoff
  \beginverse
    \beginrep
      Os an^jinhos do |^céu é quem ^vem co|^lher \altchords{\id[4]{MK (C)}|C |Am}
      Para ^ajun|^tar em um ^{só lu}|^gar \altchords{|G F |C}
    \endrep
    \beginrep
      ^ Lou|^var a meu ^Pai E|^terno \altchords{|G F |Am}
      Fel^iz da|^quele que bem ^traba|^lhar \altchords{|G F |C}
    \endrep
  \endverse
  \begin{translation}[EN]
    When you hear it said that St. John is on the earth
    It's a sign of war everywhere
    \nextverse
    Apart from the confusion
    St. Peter arrived with his two brothers
    \nextverse
    The little angels of heaven are the ones who come to gather
    To bring together in one single place
    \nextverse
    Praise to my Eternal Father
    Happy is he who works well
  \end{translation}
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
    % transcribed by larva, latest update on 2026-04
    theMelody = \relative g' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 4/4 \partial 2.
      \sectionLabel "A"
      \repeat volta 2 {
         g8\pomark g c4 e | g8( g) g e a4 g
         | e8( c) c a c4 e | d8( d) d c a4 c
      } \alternative {
        { | c4 }
        { | c2. }
      }
      \sectionLabel "B"
      \repeat volta 2 {
         c4 | d d8 c a4 c | e4( e8) a,8 c4 e4
         | d8 d d c a4 c4
      } \alternative {
        { | c2. }
        { | c4 }
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1."
      \repeat volta 2 {
        Quan -- do~ou -- vir fa -- | lar de São Jo -- ão na | ter -- ra;
        É si -- nal de | guer -- ra em to -- do lu --
      } \alternative {
        { | gar; }
        { | gar. }
      }
      \repeat volta 2 {
        É | fo -- ra de con -- fu -- | são; _
        Che -- gou São | Pe -- dro com seus dois ir --
      } \alternative {
        { | mãos; }
        { | mãos. }
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2."
      \repeat volta 2 {
        Os an -- jinhos do | céu _ é quem vem co -- | lher; _
        Pa -- ra a -- jun -- | tar _ em um só lu --
      } \alternative {
        { | gar; }
        { | gar. }
      }
      \repeat volta 2 {
        Lou -- | var a meu Pai E -- | ter -- no;
        Fe -- liz da -- | que -- le que bem tra -- ba --
      } \alternative {
        { | lhar; }
        { | lhar. }
      }
    }
    theChords = \chordmode {
      \repeat volta 2 {
        s2. | c1 | a2:m c | d:m f
      } \alternative {
        { | c4 }
        { | c2. }
      }
      \repeat volta 2 {
        s4 | d1:m | c | d2:m f
      } \alternative {
        { | c2. }
        { | c4 \bar "|." }
      }
    }
    \include "ulsbs-include-tail-lyricsbetween.ly"
  \end{lilypond}\end{lilywrap}
\endsong


\beginsong{Peço que Vós me Ouça}[id={peco-que-vos-me-ouca-s0epoa},by={Sebastião},tags={valsa, 2x},key={C},gk={C, C--D}]
  % Also in songs_astral_oracao.tex
  \audio[]{https://www.nossairmandade.com/hymn/10/Pe\%C3\%A7oQueV\%C3\%B3sMeOu\%C3\%A7a}
  \audio[key={C},title={Ceu da Santa Luz}]{https://www.youtube.com/watch?v=wKMY4YGdYeY&t=1564s}
  % Note: This hino is sometimes sung two times, but not in Oração
  \newchords{chords_cura_pecoquevosmeouca_a}\newchords{chords_cura_pecoquevosmeouca_b}
  \meter{3}{4}
  \mnbeginverse
    \beginrep\memorize[chords_cura_pecoquevosmeouca_a]
      \[^<G>]Meu |\[<C>C]Pai \[^<F>]pe\[^<E>]ço |\[<D>G]que \[^<F>]Vós \[^<E>]me |\[<C>C]ou|\[C7]ça \altchords{\id[1]{(D)}|D |A |D |D7}
      |\[<A>Fmaj7]Para eu pe|\[<G>C]dir \[^<C>]o \[^<E>]per|\[<G>G]dão | \e \altchords{|Gmaj7 |D |A | \e}
    \endrep
    \mnendverse\glueverses\mnbeginverse
    \beginrep\memorize[chords_cura_pecoquevosmeouca_b]
      \[^<C>]Eu |\[<A>F]peço não |\[<G>G]só \[^<F>]pa\[^<E>]ra |\[<C>C]mi|\[Am]m \altchords{|G |A |D |Bm}
      \[^<A>]Pa\[^<C>]ra |\[<E>C]mim \[^<G>]e |\[<D>G7]os \[^<F>]meus \[^<E>]ir|\[<C>C]mãos |\[\ac{1}{C7}]{} \e \altchords{|D |A7 |D |\ac{1}{D7}}
    \endrep
  \mnendverse
  \notesoff
  \beginverse\replay[chords_cura_pecoquevosmeouca_a]
    \beginrep
      Meu |^Pai quando |^for perdo|^ar |^ \e
      Per|^{doa co}|^mo lhe con|^vém | \e
    \endrep
    \endverse\glueverses\beginverse\replay[chords_cura_pecoquevosmeouca_b]
    \beginrep
      Eu |^peço que |^vós nos per|^do|^e
      Como |^perdo|^ou em Be|^lém |^ \e
    \endrep
  \endverse
  \beginverse\replay[chords_cura_pecoquevosmeouca_a]
    \beginrep
      A |^barca que |^corre no |^mar |^ \e \altchords{\id[4]{MK (C)}|C |G7 |C |C7}
      |^Corre no |^meu cora|^ção | \e \altchords{|Fmaj7 |C |G |\ac{1}{G7} \ac{2}{C}}
    \endrep
    \endverse\glueverses\beginverse\replay[chords_cura_pecoquevosmeouca_b]
    \beginrep
      A|^quele que |^aqui ba|^ti|^za \altchords{|F |G |Am |F}
      Bati|^zou {\noteson\[<G>]no \[<F>]ri}|^{o de} Jor|^dão |^ \e \altchords{|C |G7 |C |\ac{1}{C7}}
    \endrep
  \endverse
  \begin{lilywrap}\begin{lilypond}[] \include "ulsbs-include-head.ly"
    % transcribed by larva, updated on 2023-02
    theMelody = \relative g' {
      \set melismaBusyProperties = #'() \slurDashed
      \key c \major \time 3/4 \partial 4
      \sectionLabel "A"
      \repeat volta 2 {
        g4\pomark | c4 f4 e4 | d4 f4 e4 | c2.( | c2)( c4)
        | a'4 a4 a4 | g4 c,4 e4 | g2.~ | g2
      }
      \sectionLabel "B"
      \repeat volta 2 {
        c,4 | a'4 a4 a4 | g4 f4 e4 | c2.(
        | c4) a4 c4
        \set melismaBusyProperties = #'() \slurDown
        | e4( << e4) \single\altcol g4\altmark "(3.)" >> << g4 \single\altcol f4 >> | d4 f4 e4 | c2.~ | c2
      }
    }
    theLyricsOne = \lyricmode {
      \set stanza = "1.A"
      \repeat volta 2 {
        Meu | Pai pe -- ço | que Vós me | ou -- | ça; __ _
        | Pa -- ra~eu pe -- | dir o per -- | dão. __ _
      }
      \set stanza = "1.B"
      \repeat volta 2 {
        Eu | pe -- ço não | só pa -- ra | mim; __ _
        Pa -- ra | mim __ _ e | os meus ir -- | mãos. __ _
      }
    }
    theLyricsTwo = \lyricmode {
      \set stanza = "2.A"
      \repeat volta 2 {
        Meu | Pai quan -- do | for per -- do -- | ar; __ _
        Per -- | do -- a co -- | mo lhe con -- | vém. __ _
      }
      \set stanza = "2.B"
      \repeat volta 2 {
        Eu | pe -- ço que | vós nos per -- |do -- | e;
        Co -- mo | per -- __ _ do -- | ou em Be -- | lém. __ _
      }
    }
    theLyricsThree = \lyricmode {
      \set stanza = "3.A"
      \repeat volta 2 {
        A | bar -- ca que | cor -- re no | mar; __ _ _
        | Cor -- re no | meu co -- ra -- | ção. __ _
      }
      \set stanza = "3.B"
      \repeat volta 2 {
        A -- | que -- le que | a -- qui ba -- | ti -- |za;
        Ba -- ti -- | zou \altcol no \altcol ri -- | o de Jor -- | dão. __ _
      }
    }
    theChords = \chordmode {
     \repeat volta 2 {
       s4 | c2. | g | c | c:7
       | f:maj7 | c | g | g2
     }
     \repeat volta 2 {
       s4 | f2. | g | c | a:m
       | c | g:7 | c | \mark \markup{\hspace #1 \super{"1"}} \parenthesize c2:7
     }
    }
    \setStaffSize #13 % for better fit
    \include "ulsbs-include-tail.ly"
  \end{lilypond}\end{lilywrap}
  \begin{translation}[EN]
    My Father I ask that You hear me, for me to ask for forgiveness
    I ask not only for me, but for me and my brothers and sisters
    \nextverse
    My Father when He forgives, he forgives as it befits Him
    I ask that You forgive us, as You forgave in Bethlehem
    \nextverse
    The boat which sails on the sea sails within my heart
    The one who baptizes here, is baptized in the Jordan River
  \end{translation}
\endsong
