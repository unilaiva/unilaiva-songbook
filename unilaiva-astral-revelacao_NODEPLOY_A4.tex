 %% unilaiva-astral-revelacao
 %% =========================
 %%
 %% Hinario Revelação by Simja Johannes Lempinen
 %%

% Setup the book name (the subtitle), and include common settings
\newcommand{\subbooktitle}{Revelação}
\newcommand{\papersize}{a4paper}
\newcommand{\basicfontsize}{12pt}
% The following creates space between Lilypond systems. Use star version
% of \vspace to avoid page breaks between systems. The default is \linebreak
% without extra \vspace.
\newcommand{\betweenLilyPondSystem}[1]{\vspace*{.5ex}\linebreak}

\newcommand{\secondpagefootnote}{%
  The hinos in this book are transcribed and made public by larva with\\
  the permission and help of their author Simja Johannes Lempinen.\\
  This is a work in progress: the transcriptions still contain mistakes.
}

% Include common settings for astral books
\input{tex/unilaiva-astral_setup_include.tex}

% fancyhdr: setup header (modify the existing unilaiva style)
\fancypagestyle{unilaiva}{%
  \fancyhead[CE]{\rmfamily\small\itshape{\mainbooktitle}} % "Unilaiva no Astral"
  \fancyhead[RE]{}
  \fancyhead[LO]{}
  \fancyhead[CO]{\rmfamily\small\itshape{\leftmark}} % odd center: chapter name
}

% songs: create indexes (these must be named exactly like this!)
\newindex{titleidx}{idx_unilaiva-astral-revelacao_NODEPLOY_A4_title}
\newauthorindex{authidx}{idx_unilaiva-astral-revelacao_NODEPLOY_A4_auth}
\newscripindex{tagidx}{idx_unilaiva-astral-revelacao_NODEPLOY_A4_tag}

\renewcommand{\bookbytext}{%
  Received by:\par\vspace{0.618034ex}\textbf{\Large Simja Johannes Lempinen}%
}%

\begin{document}

  % Cover page (recto)
  \includepdf[]{content/img/unilaiva-astral-revelacao_COVER.pdf}
  % The second page (verso):
  \secondpage

  % TOC:
  \toc

  % Sets chapter number; will be incremented by one at each \mainchapter,
  % \songchapter or \chapter:
  \setcounter{chapter}{0}

  \mainchapter[]{Simja Johannes Lempinen: Revelação}
    \begin{songs}{}
      \input{content/songs_astral_revelacao.tex}
    \end{songs}

  % Here start appendices: chapter numbering resets and switches to alphabetic
  \appendix
  \chapter{Technical}
    \input{content/section_technical_information.tex}

\end{document}
